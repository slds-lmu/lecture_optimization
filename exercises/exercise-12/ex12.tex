...
\item Perform one round of coordinate descent starting from an arbitrary point $(x_{1}, x_{2})$.
      Show that after updating $x_{1}$ (while fixing $x_{2}$), the algorithm arrives at a point where $x_{1} = x_{2}$ and terminates.
      That is, show that coordinate descent will not move beyond the initial update from the first iteration.
