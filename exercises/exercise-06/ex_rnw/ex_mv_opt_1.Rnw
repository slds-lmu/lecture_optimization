...
\item Solve the logistic regression via gradient descent. Use as step width = 0.5 and train for 50 steps. Explain your observation.\\
\textit{Hint:} a)
...
\item Repeat e) but with backtracking. Set $\alpha = 3, \gamma = 0.9$ and $\tau = 0.8$
\end{enumerate}
