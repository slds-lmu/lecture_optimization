...
\loesung{Newton-Raphson and Gauss-Newton}{
...
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlnum{30}\hlstd{)\{}
  ...
  \hlstd{res} \hlkwb{=} \hlstd{(y}\hlopt{-}\hlnum{1}\hlopt{/}\hlstd{(}\hlnum{1}\hlopt{+}\hlstd{exp_f))}
  \hlstd{grad_res} \hlkwb{=} \hlkwd{c}\hlstd{(exp_f} \hlopt{/} \hlstd{(exp_f} \hlopt{+} \hlnum{1}\hlstd{)}\hlopt{^}\hlnum{2}\hlstd{)} \hlopt{*} \hlstd{X_model}
