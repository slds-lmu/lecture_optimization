...
\item Show that the second primal form of the linear SVM $\min_{\mathbf{\bm{\theta}, \bm{\theta}_0}} \sum^n_{i=1}\max(1-y^{(i)}(\bm{\theta}^\top\mathbf{x}^{(i)} + \bm{\theta}_0), 0) + \lambda\Vert\bm{\theta}\Vert^2_2$ is a convex problem.
\item \textit{Bonus:} Show that the first primal form of the linear SVM with soft constraints \\
