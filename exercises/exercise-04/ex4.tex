...
Show that if $\lambda > 0$ then $\Vert\bm{\theta}_{\text{reg}}^*\Vert_2 = t(\lambda) < \Vert\bm{\theta}^*\Vert_2$ where $\bm{\theta}^*$ and $\bm{\theta}_{\text{reg}}^*$ are the minimzier of unregularized regression and the ridge regression, respectively. \\ \textit{Hint 1}: You do not need to find $t(\lambda)$ \\
...
From this it follows that $\P(y = y^{(i)}|\mathbf{x}^{(i)}) = \P(y = y^{(i)}|\mathbf{x}^{(i)})^{y^{(i)}}(1-\P(y = y^{(i)}|\mathbf{x}^{(i)})^{1-y^{(i)}}.$ With this derive the empirical risk $\mathcal{R}_{\text{emp}}$ as shown in the lecture following the maximum likelihood principle. (Assume the observations are independent)
...
$\min_{\mathbf{\bm{\theta}, \bm{\theta}_0}} \frac{1}{2}\Vert\bm{\theta}\Vert^2_2 + C\sum^n_{i=1}\zeta^{(i)}$ s.t. 
