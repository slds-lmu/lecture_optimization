...
\begin{enumerate}
  \setcounter{enumi}{3}
	\item Let $\mathcal{R} = \{r\in\R|\;r>0, f_{|S_r} \text{ is convex}\}.$ Find $\overline{r} = \sup \mathcal{R}.$
	\item Find the local minimum $\mathbf{x}^*$ of $f_{\overline{r}}$
...
\begin{enumerate}
	...
	\item Compute $\nabla f$ and $\nabla^2 f$
	\item Find the minimizer $\mathbf{w}^*$ of $f$. Is this a global minimum?
