\begin{enumerate}
  \item We compute both expression and compare the results.
    \begin{align*}
      ...
      &= \E_\mathbf{x} \E_{y|\mathbf{x}} 2\mathbf{x} \mathbf{x}^\top \bm{\theta} - 2\mathbf{x}y = \E_\mathbf{x} 2\mathbf{x} \mathbf{x}^\top \bm{\theta} - 2\mathbf{x}\mathbf{x}^\top \bm{\theta}^* \\
      &= 2\bm{\Sigma}_\mathbf{x}(\bm{\theta} - \bm{\theta}^*)
    \end{align*}
  ...
  &= \nabla_{\bm{\theta}}(\bm{\theta}^\top\bm{\Sigma}_\mathbf{x}\bm{\theta} - 2\bm{\theta}^\top\bm{\Sigma}_\mathbf{x} \bm{\theta}^*  + {\bm{\theta}^*}^\top\bm{\Sigma}_\mathbf{x} \bm{\theta}^*) \\
  &= 2\bm{\Sigma}_\mathbf{x}(\bm{\theta} - \bm{\theta}^*)
    \end{align*}
