...
\begin{enumerate}
\item Determine which of these points are Pareto optimal (find $\mathcal{P}$).
\item Sketch the objective space and indicate the Pareto front $f(\mathcal{P})$.
...
\item Compute the crowding distance of the point $\mathbf{x}^{(3)}$ with the solution $\mathbf{f}^{(3)}$.
\item Compute the hypervolume contribution of the point $\mathbf{x}^{(5)}$ with the solution $\mathbf{f}^{(5)}$. Again, assume a reference point $R = (15, 15)$.
\end{enumerate}
