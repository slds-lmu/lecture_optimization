...
\hlstd{g} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{u}\hlstd{) \{}\hlkwd{ifelse}\hlstd{(}\hlkwd{abs}\hlstd{(u)} \hlopt{<} \hlnum{1}\hlstd{, (}\hlnum{1} \hlopt{-} \hlstd{u}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{^}\hlnum{2}\hlstd{,} \hlnum{0}\hlstd{)\}}

\hlstd{f1} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{) \{}\hlkwd{sqrt}\hlstd{((x} \hlopt{-} \hlstd{c2)}  \hlopt{%*%} \hlstd{S2_inv} \hlopt{%*%} \hlstd{(x} \hlopt{-} \hlstd{c2))\}}
\hlstd{f2} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{) \{}\hlkwd{sqrt}\hlstd{((x} \hlopt{-} \hlstd{c1)} \hlopt{%*%} \hlstd{(x} \hlopt{-}\hlstd{c1))\}}
\hlstd{f} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{) \{}\hlopt{-}\hlkwd{g}\hlstd{(}\hlkwd{f1}\hlstd{(x))} \hlopt{+} \hlkwd{g}\hlstd{(}\hlkwd{f2}\hlstd{(x))\}}
...
\item $\Vert\mathbf{x} - \mathbf{c}_2\Vert_{S_2}^2 = (\mathbf{x} - \mathbf{c}_2)^\top \begin{pmatrix} 1.1 & -0.9 \\ -0.9 & 1.1\end{pmatrix}^{-1} (\mathbf{x} - \mathbf{c}_2)$ (ellipse around $\mathbf{c}_2$)\\
