...
\begin{framev}[fs=footnotesize]{Why Constraints Break GD}
\splitV[0.46]{
\textbf{Problem}
$$
\min_{\xv \in \S} \fx, \text{ with } \S \subsetneq \R^d
$$
...
\item Roadmap: feasible updates first, relaxation next, then hybrid methods.
\end{itemize}
}{
\imageC[0.92]{figure/primal_gd_projected_step.pdf}
}
\end{framev}
...
\item Equivalently, the negative gradient must lie in the normal cone:
$$
-\nabla f(\xvs) \in N_{\S}(\xvs).
$$
...
\begin{framev}[fs=scriptsize]{Toy Example: Box Constraints}
\textbf{Toy problem}
$$
\min_{\wv \in \S}
F(\wv)
...
\textbf{Projection}
\begin{align*}
\left[\Pi_{\S}(\zv)\right]_1 &= \min\{1, \max\{0, z_1\}\},\\
\left[\Pi_{\S}(\zv)\right]_2 &= \min\{0.9, \max\{0, z_2\}\}.
\end{align*}

\begin{itemize}
\item The unconstrained minimizer is $(1.2,0.2)$, so it lies just outside the box.
\item A projected-GD step repairs only the violated coordinates by clipping.
\item We reuse this same box-constrained quadratic in the next sections.
\end{itemize}
}{
