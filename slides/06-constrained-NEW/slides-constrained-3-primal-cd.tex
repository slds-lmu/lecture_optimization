...
\imageC[1]{figure/primal_cd_intervals.pdf}
}
\end{framev}


\begin{framei}[fs=small]{Constrained Coordinate Descent}
\item Start from a feasible point $\xv^{[0]} \in \S$ and choose the active coordinate $i_t$ cyclically or at random.
\item For the current iterate, compute the feasible slice
$$
I_t := \left\{ z \in \R \mid (\xv_{-i_t}^{[t]}, z) \in \S \right\},
$$
where $\xv_{-i_t}^{[t]}$ denotes all coordinates except the active one.
\item Solve the one-dimensional constrained problem
$$
x_{i_t}^{[t+1]} \in \argmin_{z \in I_t} F(\xv_{-i_t}^{[t]}, z).
$$
\item Keep all other coordinates unchanged:
$$
x_j^{[t+1]} = x_j^{[t]} \qquad \forall j \neq i_t.
$$
...
\item For a feasible point $\xv$ and coordinate $i$, define the slice
$$
I_i(\xv) := \left\{ z \in \R \mid (x_1,\dots,x_{i-1},z,x_{i+1},\dots,x_d) \in \S \right\}.
$$
\item Geometrically, $I_i(\xv)$ is the intersection of $\S$ with an affine line parallel to axis $i$.
\item If $\S$ is convex, then $I_i(\xv)$ is a convex subset of $\R$, hence an interval.
...
\begin{framei}[fs=scriptsize]{Toy Example: One Coordinate Slice}
\item Reuse the box-constrained quadratic
\[
\min_{\wv \in [0,1] \times [0,0.9]}
F(\wv)
=
(w_1 - 1.2)^2
+
...
(w_2 - 0.2)^2.
\]
\item Start from the feasible point $\wv^{[t]} = (0.2,0.8)^\top$ and update only $w_1$.
\item The 1D subproblem is
\[
\min_{0 \le z \le 1} F(z,0.8)
=
\min_{0 \le z \le 1}
\Big((z - 1.2)^2 + 0.6(z - 1.2) + 0.36\Big).
\]
\item Stationarity gives
\[
\frac{d}{dz}F(z,0.8) = 2(z - 1.2) + 0.6 = 0
\qquad \Longrightarrow \qquad
\hat z = 0.9.
\]
\item Because $\hat z \in [0,1]$, the exact coordinate update is $w_1^{[t+1]} = 0.9$.
\item The next $w_2$ step solves $w_2 \leftarrow \Pi_{[0,0.9]}(0.8 - 0.5 w_1)$ and moves toward $\wv^\ast = (1,0.3)^\top$.
\end{framei}
...
\item In practice, exact minimization is often replaced by a sufficiently improving approximate coordinate step.
...
\imageC[1]{figure/primal_cd_path.pdf}
}
\begin{itemize}
\item The interval does not depend on the other coordinates.
\item On the toy example, the exact updates are
\[
w_1 \leftarrow \Pi_{[0,1]}(1.3 - 0.5 w_2),
\qquad
w_2 \leftarrow \Pi_{[0,0.9]}(0.8 - 0.5 w_1).
\]
...
\item \textbf{Special case}: nonnegative least squares is recovered with $l_i = 0$, no finite upper bounds, and $\lambda = 0$.
\end{framei}
...
\imageC[0.85]{figure/primal_cd_truncation.pdf}
}
\end{framev}


\begin{framei}[fs=small]{Practical Remarks}
\item Coordinate descent is attractive when each slice $I_i(\xv)$ is easy to compute and the one-dimensional minimizer is cheap.
...
\item In large linear models, the residual vector can be maintained incrementally after each coordinate update instead of recomputing $\mathbf{D}\wv$ from scratch.
...
\end{enumerate}
\oneliner{Primal coordinate descent is most useful when constraint structure makes each coordinate slice simple, especially for box-type constraints such as our toy problem.}
\end{framev}
