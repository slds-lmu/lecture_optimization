...
plotObjectiveSpace2Crit = function(smoof.fun) {
  ...
  names(des.eval.df) = c("f1", "f2")
  
  p = ggplot() + geom_point(data = des.eval.df, aes(x = f1, y = f2), size = 0.7, color = "grey", alpha = 0.2)
...
ggsave(grid.draw(g), file = "figure_man/NSGA2_steps.png", width = 8, height = 4)
...
popdf$Front = factor(sorted$ranks, ordered = TRUE, levels = 1:4)

pl = p + geom_point(data = popdf[popdf$Front %in% 1:4, ], aes(x = X1, y = X2, colour = Front)) 
pl = pl + geom_line(data = popdf[popdf$Front %in% 1:4, ], aes(x = X1, y = X2, colour = Front), lty = 2)
ggsave(pl, file = "figure_man/NSGA2_NDS.png", width = 4, height = 3)


# Crowd Sort - Example 1
F3 = popdf[which(popdf$Front == max(popdf$Front)), ]
...
pl1 = pl + geom_point(data = F3[order(cd)[1:4], ], aes(x = X1, y = X2), size = 3, shape = 17) 
...
ggsave(grid.draw(g), file = "figure_man/NSGA2_CS1.png", width = 6, height = 3)


cdo = order(cd, decreasing = TRUE)[c(3, length(cd))]
