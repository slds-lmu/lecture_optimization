...
\imageC[1]{figure/ea_recombination_numeric.pdf}
}
\end{framev}
...
\comment[NOTE]{BB}{ein satz begründung bitte zu polynomial aus deb paper}
\end{itemize}
\vfill
\imageC[0.45][https://www.researchgate.net/publication/262320509_Analysing_mutation_schemes_for_real-parameter_genetic_algorithms]{figure_man/polynomial_mutation.png}
\end{framev}


\begin{framev}[align=top, fs=small]{Example of an evolutionary algorithm}
...
\imageFixed{2.5cm}{2.5cm}[0.8]{figure/1dim-ackley-func.png}
\end{framev}


\begin{framev}[align=top]{Example of an evolutionary algorithm}
Random initial population with size $\mu = 10$
\imageFixed{2.5cm}{2.5cm}[0.8]{figure/1dim-ackley-func-2.png}
\end{framev}


\begin{framev}[align=top]{Example 1: Ackley function}
We choose $\lambda = 5$ offspring by neutral selection (red individuals).
\imageFixed{2.5cm}{2.5cm}[0.8]{figure/1dim-ackley-func-neutral-selec.png}
\end{framev}


\begin{framev}[align=top]{Example 1: Ackley function}
Use Gaussian mutation with $\sigma=2$, but without recombination.
\imageFixed{2.5cm}{2.5cm}[0.8]{figure/1dim-ackley-func-gaussian-mutation.png}
\end{framev}
...
\imageFixed{2.5cm}{2.5cm}[0.8]{figure/1dim-ackley-func-selection.png}
\end{framev}
...
\comment[NOTE]{BB}{das ist doch quatsch. erstens ist der radius hier nicht kodiert. den kann man maximal wählen und sich das in der kodierungs sparen.  aber dann ist das bild von vorher falsch. außerdem brauchen wir da auch keine shiny app sondern sollten das lieber auf den slides machen}
\spacer
...
\comment[NOTE]{BB}{generell sollte hier noch ein 2d beispiel mit recombo.}
