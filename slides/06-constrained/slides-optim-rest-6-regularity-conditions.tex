...
0 \\ 0 \end{pmatrix}
$$
\item For this and the general Lagrangian, this leads to the following question
\item[\textbf{Question:}] Is $\nabla L(\xv, \bm{\alpha}, \bm{\beta}) = 0$ a \textbf{necessary / sufficient condition for the optimum}?
\end{framei}
...
\item \sourceref{https://docs.ufpr.br/~ademir.ribeiro/ensino/cm721/kkt.pdf}
\end{framei}
...
\begin{equation}
\min_{\bm{\theta}} \quad f_\lambda(\bm{\theta}) := \|\bm{y} - \mathbf{X}\bm{\theta}\|_2^2 + \lambda \|\bm{\theta}\|_2^2
\label{eq:form1}
\end{equation}
...
\vfill
\imageC[0.65]{figure_man/ridge_original.png}
...
\item Consider \eqref{eq:form1}. If $\bm{\theta}^*$ is our minimum, the necessary condition applies:
$$
\nabla f_\lambda(\bm{\theta}^*) = - 2 \bm{y}^T\mathbf{X} + 2 (\bm{\theta}^*)^T \mathbf{X}^T\mathbf{X} + 2 \lambda (\bm{\theta}^*)^T = 0
$$
\item We show that we can find a $t$ so that $\bm{\theta}^*$ is also solution for \eqref{eq:form2}
...
$$
\item Since $\nabla f_\lambda(\bm{\theta}^*) = 0$, this is fulfilled if we set $\bm{\theta} = \bm{\theta}^*$ and $\alpha = \lambda$
\end{framei}
...
$$
\item This is the case if we choose $t = \|\bm{\theta}^*\|^2$
\vfill
