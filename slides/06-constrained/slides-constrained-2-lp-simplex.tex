...
\item General form
$$
\min_{\xv} \mathbf{c}^T\xv \quad \text{s.t. } \Amat_1\xv \le \mathbf{b}_1,\ \Amat_2\xv \ge \mathbf{b}_2,\ \Amat_3\xv = \mathbf{b}_3
$$
\item Examples
\splitV[0.5]{
$$
...
$$
$$
\min_{\xv,\mathbf{s}} \one^T\mathbf{s} \quad \text{s.t. } \pm(\Amat\xv - \mathbf{b}) \le \mathbf{s}
$$
}{
$$
...
\item[] Standard Form
$$
\min \mathbf{c}^T\xv \quad \text{s.t. } \Amat\xv \le \mathbf{b},\ \xv \ge \zero
$$
...
\item $\min_{\xv} \mathbf{c}^T\xv \Leftrightarrow \min_{\xv^+, \xv^-} \begin{bmatrix} \mathbf{c}^T & -\mathbf{c}^T \end{bmatrix} \begin{bmatrix} \xv^+ \\ \xv^- \end{bmatrix}$
\end{framei}
...
\item Equality Form
$$
\min \mathbf{c}^T\xv \quad \text{s.t. } \Amat\xv = \mathbf{b},\ \xv \ge \zero
$$
...
\item $\min_{\xv} \mathbf{c}^T\xv \Leftrightarrow \min_{\xv,\mathbf{s}} \begin{bmatrix} \mathbf{c}^T & \zero^T \end{bmatrix} \begin{bmatrix} \xv \\ \mathbf{s} \end{bmatrix}$
\end{framei}


\begin{framei}{Geometric interpretation}
\item[] \textbf{Feasible set}
\item Points $\{\xv: \Amat_i^T \xv = b_i\}$ form a hyperplane in $\R^n$\\
$\Amat_i$ is perpendicular to the hyperplane and called \textbf{normal vector}
\item Points $\{\xv: \Amat_i^T \xv \le b_i\}$ lie on one side of the hyperplane, which form a convex half-space
...
\end{enumerate}
\imageC[0.8]{figure/cons-solutions-lp.png}
\end{framei}


\begin{framei}{Geometric interpretation}
\item[] \textbf{Case 3: LP is feasible and bounded}
...
\vfill
\imageC[0.3]{figure/cons-opposite-direction.png}
\end{framei}
...
$$L(\xv, \muv, \bm{\lambda}) = \mathbf{c}^T\xv - \muv^T\xv - \bm{\lambda}^T(\Amat\xv - \mathbf{b})$$
with $\muv \ge \zero$
\item Optimal solution satisfies $\frac{\partial L}{\partial \xv}=0$, i.e., $\Amat^T \bm{\lambda} + \muv = \mathbf{c}$
\item Decompose stationarity condition into $\mathcal{B}$ and $\mathcal{V}$ components:
$$
\Amat^T \bm{\lambda} + \muv = \mathbf{c} 
\quad \implies \quad
\begin{cases}
\Amat_{\mathcal{B}}^T \bm{\lambda} + \muv_{\mathcal{B}} = \mathbf{c}_{\mathcal{B}} \\
\Amat_{\mathcal{V}}^T \bm{\lambda} + \muv_{\mathcal{V}} = \mathbf{c}_{\mathcal{V}}
\end{cases}
$$
\end{framei}
...
\item Choose $ \muv_{\mathcal{B}} = 0 $ to satisfy $\muv \odot \xv = 0$, since for optimality, we need $\mu_i=0$ when $x_i > 0$
\item Compute $ \bm{\lambda} $ from $ \mathcal{B} $:
$$
\Amat_{\mathcal{B}}^T \bm{\lambda} + \underbrace{\muv_{\mathcal{B}}}_{= 0} = \mathbf{c}_{\mathcal{B}} \quad \implies \quad \bm{\lambda} = \Amat_{\mathcal{B}}^{-T} \mathbf{c}_{\mathcal{B}}
$$
\item Use this to obtain:
\begin{align*}
\Amat_{\mathcal{V}}^T \bm{\lambda} + \muv_{\mathcal{V}} &= \mathbf{c}_{\mathcal{V}}\\
\muv_{\mathcal{V}} &= \mathbf{c}_{\mathcal{V}} - \Amat_{\mathcal{V}}^T \bm{\lambda}\\
\muv_{\mathcal{V}} &= \mathbf{c}_{\mathcal{V}} - \left( \Amat_{\mathcal{B}}^{-1} \Amat_{\mathcal{V}} \right)^T \mathbf{c}_{\mathcal{B}}
\end{align*}
\item Knowing $ \muv_{\mathcal{V}} $ allows us to assess optimality of the vertices\\
If $ \muv_{\mathcal{V}} $ contains negative components, then $\muv \ge \zero$ is not satisfied and the vertex is suboptimal
\end{framei}
...
\begin{framei}{Simplex algorithm: Effect on objective}
\item Effect of a transition on the obj. function can be computed using $x'_q$
\item Objective function value at the new vertex:
\begin{align*}
\mathbf{c}^T \xv' &= \mathbf{c}_{\mathcal{B}}^T \xv'_{\mathcal{B}} + c_q x'_q \\
&= \mathbf{c}_{\mathcal{B}}^T \left( \xv_{\mathcal{B}} - \Amat_{\mathcal{B}}^{-1} \Amat_{\{q\}} x'_q \right) + c_q x'_q \\
&= \mathbf{c}_{\mathcal{B}}^T \xv_{\mathcal{B}} - \mathbf{c}_{\mathcal{B}}^T \Amat_{\mathcal{B}}^{-1} \Amat_{\{q\}} x'_q + c_q x'_q \\
&= \mathbf{c}_{\mathcal{B}}^T \xv_{\mathcal{B}} - (c_q - \mu_q) x'_q + c_q x'_q \\
&= \mathbf{c}^T \xv + \mu_q x'_q
\end{align*}
using $\bm{\lambda} = \Amat_{\mathcal{B}}^{-T} \mathbf{c}_{\mathcal{B}}$ and $\Amat_{\{q\}}^T\bm{\lambda}=c_q-\mu_q$
\end{framei}


\begin{framei}{Simplex algorithm: Optimality condition}
\item Choosing an entering index $q$ decreases the obj. function value by
$$
\mathbf{c}^T \xv' - \mathbf{c}^T \xv = \mu_q x'_q
$$
...
\item If all components of $\muv_{\mathcal{V}}$ are non-negative, we have found a global optimum
\end{framei}


\begin{framei}[fs=small]{Simplex algorithm: Heuristics}
\item Since there can be multiple negative entries in $\muv_{\mathcal{V}}$, different heuristics can be used to select an entering index:
\item \textbf{Greedy heuristic}: choose a $q$ that maximally reduces $\mathbf{c}^T \xv$
\item \textbf{Dantzig's rule}: choose the $q$ with the most negative entry in $\muv$\\
Easy to calculate, but does not guarantee the maximum reduction in $\mathbf{c}^T \xv$\\
Also sensitive to scaling of the constraints
\item \textbf{Bland's rule}: choose the first $q$ with a negative entry in $\muv$\\
...
-4 & 2 & 0 & 1
\end{pmatrix},$
$\mathbf{b} = (9, 2)^T, \mathbf{c} = (3, -1, 0, 0)^T$\\
\spacer
\textbf{Solution:}\\
$\mathcal{V}=\{1,2\}, \mathcal{B}=\{3, 4\}$\\
$\xv_{\mathcal{B}} = \Amat_{\mathcal{B}}^{-1}\mathbf{b} = (9, 2)^T$\\
$\bm{\lambda} =  \Amat_{\mathcal{B}}^{-1}\mathbf{c}_{\mathcal{B}} = \zero$\\
$\muv_{\mathcal{V}} = \mathbf{c}_{\mathcal{V}} - \left( \Amat_{\mathcal{B}}^{-1} \Amat_{\mathcal{V}} \right)^T \mathbf{c}_{\mathcal{B}} = (3, -1)^T$\\
\spacer
$\muv_{\mathcal{V}}$ contains negative elements, so our current $\mathcal{B}$ is suboptimal.\\
\spacer
We will pivot on the negative one with $q=2, - \Amat_{\mathcal{B}}^{-1} \Amat_{\{q\}}=(1,2)^T$.\\
This causes $x_4 = 0$, so updated $\mathcal{B}=\{2, 3\}$.\\
In the second iteration, we find $\xv_{\mathcal{B}}=(1,8)^T, \bm{\lambda} = (0, -\frac{1}{2})^T, \muv_{\mathcal{V}} = (1, \frac{1}{2})^T$.\\
\spacer
This is optimal with no negative entry, thus we have $x^*=(0,1,8,0)^T$
\end{framev}
