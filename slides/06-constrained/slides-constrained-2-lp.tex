...
$$
\imageC[0.60]{figure_man/cons-linear-pro-example.png}
\end{framei}


\begin{framei}{LP examples: Quantile regression}
\item \textbf{(Sparse) Quantile regression}:
$$
\min_{\beta_0,\bm{\beta}} \frac{1}{n}\sumin \rho_\tau \left(\yi - \beta_0 -  \bm{\beta}^T\xi\right) \quad \text{s.t. } \|\bm{\beta}\|_1 \le t
$$
...
\begin{framei}{LP examples: Dantzig selector}
\item \textbf{Dantzig selector}:
$$
\min_{\bm{\beta}\in \R^p} \|\bm{\beta}\|_1 \quad \text{s.t. } \| \Xmat^T (\Xmat \bm{\beta} - \yv)\|_\infty \le \lambda
$$
where $\yv \in \R^n$, $\Xmat \in \R^{n \times p}$, and $\lambda > 0$ is a tuning parameter
...
\begin{framei}[sep=L]{LP: Standard form}
\item LPs can be formulated in \textbf{standard form}:
$$
\max_{\xv \in \R^n} \mathbf{c}^T\xv \quad \text{s.t. } \Amat\xv \le \bv, \; \xv \ge 0
$$
with $\Amat \in \R^{m\times n}$, $\bv \in \R^m$
...
\item $= \; \longleftrightarrow \; \leq,\geq$: replace $\mathbf{a}_i^T\xv=b_i$ by $\mathbf{a}_i^T\xv\ge b_i$ \textit{and} $\mathbf{a}_i^T\xv\le b_i$
\item No non-negativity constraint: replace $x_i$ by $x_i^+ - x_i^-$ with $x_i^+, x_i^- \ge 0$ (positive and negative part)
\end{framei}


\begin{framei}[sep=L]{LP: Standard form example}
\item Example:
$$
\min - x_1 - x_2 \quad \text{s.t. } x_1 + 2x_2 \le 1, \; 2x_1 + x_2 \le 1, \; x_1, x_2 \ge 0
$$
...
\item $i$-th inequality constraint: $\mathbf{a}_i^T \xv \le b_i$
\item Points $\{ \xv: \mathbf{a}_i^T \xv = b_i\}$ form a hyperplane in $\R^n$ \\
($\mathbf{a}_i$ is perpendicular to the hyperplane and called \textbf{normal vector})
\item Points $\{\xv: \mathbf{a}_i^T \xv \ge b_i\}$ lie on the side of the hyperplane into which the normal vector points (``half-space'')
...
\imageC[0.5]{figure_man/cons-linear-pro-example.png}
\end{framei}


\begin{framei}[sep=L]{Geometric interpretation: Polytopes}
...
\begin{framei}[sep=L]{Geometric interpretation: Convexity}
\item Points $\{\xv: \mathbf{a}_i^T \xv = b_i\}$ lie on the boundary of the polytope
\item Polytope $\{ \xv: \Amat \xv \le \bv\}$ is convex: For $\xv_1,\xv_2 \in \S$ and $t \in [0, 1]$
$$
\Amat(\xv_1 + t(\xv_2 - \xv_1)) = \Amat\xv_1 + t(\Amat\xv_2 - \Amat\xv_1) = (1 -t)\underbrace{\Amat\xv_1}_{\le \bv} + t\underbrace{\Amat\xv_2}_{\le \bv} \le (1 - t) \bv + t \bv = \bv
$$
\item Polytope $\{ \xv: \Amat \xv \le \bv\}$ is an $n$-\textbf{simplex}, i.e., convex hull of $n + 1$ \textit{affinely independent} points
\end{framei}
...
\imageC[0.65]{figure_man/cons-opposite-direction.png}
\end{framei}


\begin{framev}{Solutions to LP}
There are 3 cases for linear programming:
\begin{enumerate}
...
\end{enumerate}
\imageC[1]{figure_man/cons-solutions-lp.png}
\end{framev}


\begin{framei}[sep=L]{Solutions to LP}
