...
$$
\item[] \textbf{Idea:} Instead of $f$, optimize 2nd order Taylor approximation at point $\xtil$
$$
\tilde f(\xv) = f(\xtil) +  \nabla_{x} f(\xtil)^T (\xv- \xtil)+ \frac{1}{2} (\xv- \xtil)^T \nabla^2_{xx} f(\xtil) (\xv- \xtil)
$$
\item $h$ is also replaced by its linear approximation at $\xtil$
$$
\tilde h(\xv) = h(\xtil) + \nabla h(\xtil)^T(\xv- \xtil)
$$
\end{framei}


\begin{framei}{Sequential quadratic programming}
\item With $\bm{d} := (\xv- \xtil)$ we formulate the \textbf{quadratic auxiliary problem}
\begin{align*}
\min_{\bm{d}} \quad & \tilde f(\bm{d}) := f(\xtil) + \bm{d}^T \nabla_{x} f(\xtil) + \frac{1}{2} \bm{d}^T \nabla^2_{xx} f(\xtil) \bm{d} \\
\text{s.t. } \quad & \tilde h(\bm{d}) :=  h(\xtil) + \nabla h(\xtil)^T\bm{d} = 0
\end{align*}
...
$$
L(\bm{d}, \bm{\beta}) = \bm{d}^T \nabla_{x} f(\xtil) + \frac{1}{2} \bm{d}^T \nabla^2_{xx} f(\xtil) \bm{d} + \bm{\beta}^T (h(\xtil) + \nabla h(\xtil)^T\bm{d})
$$
we formulate the KKT conditions
\begin{itemize}
\item $\nabla_{\bm{d}} L(\bm{d}, \bm{\beta}) = \nabla_{x} f(\xtil) + \nabla^2_{xx} f(\xtil) \bm{d} + \nabla h(\xtil)^T\bm{\beta} = 0$
\item $h(\xtil) + \nabla h(\xtil)^T\bm{d} = 0$
\end{itemize}
\item Or in matrix notation
$$
\begin{pmatrix} \nabla^2_{xx} f(\xtil) & \nabla h(\xtil)^T \\ \nabla h(\xtil) & 0 \end{pmatrix} \begin{pmatrix} \bm{d} \\ \bm{\beta} \end{pmatrix} = - \begin{pmatrix} \nabla_{x} f(\xtil) \\ h(\xtil) \end{pmatrix}
$$
...
\item[] \textbf{Idea:} Replace constrained problem with a sequence of unconstrained problems using a \textbf{penalty function}
\item Instead of
$$
\min f(\xv) \qquad \text{s.t. } \quad h(\xv) = 0
$$
...
\item[] \textbf{Idea:} Establish a ``barrier'' that penalizes if $\xv$ comes too close to the edge of the feasible set $\bm{S}$
