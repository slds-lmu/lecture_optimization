...
$$
\min_{\xv \in \R^d} f(\xv) := \mathbf{c}^\top\xv \quad
...
and insert it into the objective: $x^\ast = 1, \; f(x^\ast) = 1$
\end{framei}
...
$$
\item $\Rightarrow$ Minimum at $\xv^* = (\pm 1, 0)^\top$
...
$$
\min_{\xv} f(\xv) := \frac{1}{2} \xv^\top \mathbf{Q} \xv \quad
\text{s.t.} \; h(\xv) := \mathbf{C} \xv - \mathbf{d} = \mathbf{0}
$$
with $\mathbf{Q} \in \R^{d\times d}$ symmetric, $\mathbf{C} \in \R^{\ell \times d}$, and $\mathbf{d} \in \R^\ell$
\item Lagrange function: $\mathcal{L}(\xv, \bm{\beta}) = \frac{1}{2} \xv^\top \mathbf{Q} \xv + \bm{\beta}^\top (\mathbf{C} \xv - \mathbf{d})$
\item Solve
$$
...
\end{itemize}
\item \textbf{Weak duality:} $f(\xv^*) \geq g(\bm{\alpha}^*, \bm{\beta}^*)$
