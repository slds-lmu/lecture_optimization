...
\item[] \textbf{Example}: Maximum Likelihood Estimation
\item For data $(\xv^{(1)}, \ldots, \xv^{(n)})$, we want to find the MLE
$$
\max_\theta L(\theta) = \prodin f(\xv^{(i)}, \theta)
$$
...
$$
\min_{\bm{\beta}\in \R^p} \frac{1}{n}\sumin\left(y^{(i)} - \bm{\beta}^T\xv^{(i)}\right)^2 \quad \text{s.t. } \|\bm{\beta}\|_1 \le t
$$
\item \textbf{Ridge regression}:
$$
\min_{\bm{\beta}\in \R^p} \frac{1}{n}\sumin\left(y^{(i)} - \bm{\beta}^T\xv^{(i)}\right)^2 \quad \text{s.t. } \|\bm{\beta}\|_2 \le t
$$
\imageC[0.7]{figure_man/lasso-ridge.png}
\end{framei}
...
$$
\min_{\bm{\beta}\in \R^p} \frac{1}{n}\sumin\left(y^{(i)} - \bm{\beta}^T\xv^{(i)}\right)^2 \quad \text{s.t. } \|\bm{\beta}\|_1 \le t, ~ \mathbf{C}\bm{\beta} \le \mathbf{d}, ~ \Amat\bm{\beta} = \mathbf{b}
$$
...
$$
\max_{\boldsymbol{\alpha} \in \R^n} \sumin \alpha_i - \frac{1}{2}\sumin\sumjn\alpha_i\alpha_j\yi y^{(j)} \scp{\xi}{\xv^{(j)}}
$$
$$
\text{s.t. } 0 \le \alpha_i \le C, \quad \sumin \alpha_i \yi = 0
$$
\end{framei}
...
$$
\S := \{\xv \in \R^d ~|~ g_i(\xv) \le 0, h_j(\xv) = 0 ~\forall~ i, j\}
$$
\end{framei}


\begin{framei}[fs=small]{Constrained Convex Optimization}
\item \textbf{Convex programs}: convex objective $f$, convex inequality constraints $g_i$, and affine equality constraints $h_j$ (i.e., $h_j(\xv) = \Amat_j^T \xv - \mathbf{b}_j$)
\imageC[0.3]{figure_man/convex_programs.png}
...
$$
\fx = \frac{1}{2}\xv^T \mathbf{Q} \xv + \mathbf{c}^T \xv + d \quad \text{for } \mathbf{Q} \in \R^{d \times d}, \mathbf{c} \in \R^d, d \in \R
$$
