...
\item[] \textbf{Example:} A bakery sells brownies for $50$ ct and mini cheesecakes for $80$ ct each
...
$$
\textcolor{orange}{5} \cdot (3x_1) + \textcolor{orange}{5} \cdot (2x_1 + 4x_2) + \textcolor{orange}{12} \cdot (2x_1 + 5x_2) = 49 x_1 + 80 x_2 \ge 30 + 50 + 96 = 176
$$
...
&\ge 6 \textcolor{orange}{\alpha_1} + 10 \textcolor{orange}{\alpha_2} + 8 \textcolor{orange}{\alpha_3}
\end{align*}
\item \textbf{But:} We have to demand that
...
$$\max_{\bm{\alpha} \in \R^m} \quad g(\bm{\alpha}) := \bm{\alpha}^T \bv \quad \text{s.t. } \bm{\alpha}^T \Amat \le \mathbf{c}^T, \quad \bm{\alpha} \ge 0$$
\item[] \textbf{Primal problem:}
$$\min_{\xv \in \R^n} \quad \fx := \mathbf{c}^T \xv \quad \text{s.t. } \Amat \xv \ge \bv, \quad \xv \ge 0$$
\end{framei}
...
$$g(\bm{\alpha}) = \bm{\alpha}^T \bv \le \mathbf{c}^T \xv = \fx$$
...
\vfill
\item[] \textbf{Proof:}
$$\bm{\alpha}^T \bv \overset{\Amat \xv \ge \bv}{\le} \bm{\alpha}^T \Amat \xv \overset{\bm{\alpha}^T \Amat \le \mathbf{c}^T}{\le} \mathbf{c}^T \xv$$
\end{framei}
...
$$g(\bm{\alpha}^*) = (\bm{\alpha}^*)^T \bv = \mathbf{c}^T \xvs = f(\xvs)$$
\vfill
...
\begin{framei}{Alternative LP formulation}
...
\item Let $\mathbf{c} \in \R^d$, $\bv \in \R^l$, $\Amat \in \R^{l \times d}$, $\mathbf{h} \in \R^k$, and $\mathbf{G} \in \R^{k \times d}$
\item Then the primal LP is defined as
$$\min_{\xv \in \R^d} \quad \mathbf{c}^T \xv \quad \text{s.t. } \mathbf{G} \xv = \mathbf{h}, \quad \Amat \xv \le \bv$$
\end{framei}


\begin{framei}{Alternative LP formulation}
\item The corresponding dual LP:
$$\max_{\alpha \in \R^l, \beta \in \R^k} \quad -\bv^T \bm{\alpha} - \mathbf{h}^T \bm{\beta} \quad \text{s.t. } -\Amat^T \bm{\alpha} - \mathbf{G}^T \bm{\beta} = \mathbf{c}, \quad \bm{\alpha} \ge 0$$
\item The following argument again highlights the interpretation of the dual LP as a lower bound
\item Here, for $\bm{\alpha} \ge 0$ and any $\bm{\beta}$, and $\xv$ primal feasible, it holds that
$$\bm{\alpha}^T (\Amat \xv - \bv) + \bm{\beta}^T (\mathbf{G} \xv - \mathbf{h}) \le 0 \iff (-\Amat^T \bm{\alpha} - \mathbf{G}^T \bm{\beta})^T \xv \ge -\bv^T \bm{\alpha} - \mathbf{h}^T \bm{\beta}$$
\item So if $\mathbf{c} = -\Amat^T \bm{\alpha} - \mathbf{G}^T \bm{\beta}$, we get a lower bound on the primal optimal value
\end{framei}
...
\item Again, for $\bm{\alpha} \ge 0$, any $\bm{\beta}$, and $\xv$ primal feasible, it holds that
$$\mathbf{c}^T \xv \ge \mathbf{c}^T \xv + \bm{\alpha}^T (\Amat \xv - \bv) + \bm{\beta}^T (\mathbf{G} \xv - \mathbf{h}) =: \mathcal{L}(\xv, \bm{\alpha}, \bm{\beta})$$
\item If $\S$ denotes the primal feasible set, $f(\xvs)$ the primal optimal value, then for $\bm{\alpha} \ge 0$ and any $\bm{\beta}$, it holds that
$$f(\xvs) \ge \min_{\xv \in \S} \mathcal{L}(\xv, \bm{\alpha}, \bm{\beta}) \ge \min_{\xv \in \R^d} \mathcal{L}(\xv, \bm{\alpha}, \bm{\beta}) =: g(\bm{\alpha}, \bm{\beta})$$
\item This shows that $g(\bm{\alpha}, \bm{\beta})$ is a lower bound on $f(\xvs)$ for $\bm{\alpha} \ge 0$ and any $\bm{\beta}$
\end{framei}
...
$$g(\bm{\alpha}, \bm{\beta}) =
\begin{cases}
-\bv^T \bm{\alpha} - \mathbf{h}^T \bm{\beta} & \text{if $\mathbf{c} = -\Amat^T \bm{\alpha} - \mathbf{G}^T \bm{\beta}$} \\
-\infty & \text{otherwise}
\end{cases}$$
\item Maximizing $g(\bm{\alpha}, \bm{\beta})$ leads again to the first dual formulation
\item[] \textbf{Note:} Lagrangian perspective is completely general $\Rightarrow$ applicable to arbitrary (non-linear) problems
\end{framei}
...
\item We refer to the excellent course of L. Vandenberghe \sourceref{http://www.seas.ucla.edu/~vandenbe/ee236a/ee236a.html} for many more details
