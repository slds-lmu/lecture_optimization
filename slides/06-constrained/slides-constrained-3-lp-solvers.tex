...
\end{itemize}
\item In \textbf{Phase I}, a feasible corner $\xv_0$ must be found first
...
\item Find starting point $\xv_0$ by solving auxiliary LP with artificial variables $\bm{\epsilon}$:
$$
\min_{\epsilon_1, \ldots, \epsilon_m} \sumim \epsilon_i \quad \text{s.t. } \bm{Ax} + \bm{\epsilon} \ge \bm{b}, \; \epsilon_1, \ldots, \epsilon_m \ge 0, \; \xv \ge 0
$$
\item A feasible starting point for the auxiliary problem is $\xv = \zero$ and $\epsilon_i = \begin{cases} 0 & \text{if } b_i < 0 \\
b_i & \text{if } b_i \ge 0
\end{cases}$
...
\item If we find $\bm{\epsilon} = \zero$, we have a valid starting point
\item Otherwise, the original problem has no feasible solution
\end{framei}


\begin{framei}[sep=L]{Simplex -- Example}
...
\begin{small}
$$
\text{s.t. } x_1 - x_2 \ge -0.5, \; -x_1 - 2x_2 \ge -2, \; -2x_1 - x_2 \ge -2, \; -x_1 + x_2 \ge -0.5, \; \xv \ge 0
$$
\end{small}
...
\imageC[0.6]{figure/simplex_implementation/iter1.png}
\end{framev}


\begin{framev}{Simplex -- Example}
\imageC[0.6]{figure/simplex_implementation/iter2.png}
\end{framev}


\begin{framev}{Simplex -- Example}
\imageC[0.6]{figure/simplex_implementation/iter3.png}
\end{framev}


\begin{framev}{Simplex -- Example}
\imageC[0.6]{figure/simplex_implementation/iter4.png}
\end{framev}
