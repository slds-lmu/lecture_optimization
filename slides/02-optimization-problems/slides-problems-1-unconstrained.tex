...
In the above example, we can replace 

\begin{itemize}
	\item the linear model $\fxt = \thetab^\top \xv$ by any model $\fxt$ (e.g., something more complex and nonlinear)
	\item the L2-loss $\Lxy = \left(\fxt - y\right)^2$ by any loss $\Lxy$
\end{itemize}
...
\begin{vbframe}{Example 3: Linear model with Hinge loss}


\end{vbframe}

\begin{vbframe}{Example 4: Regularized linear model}
