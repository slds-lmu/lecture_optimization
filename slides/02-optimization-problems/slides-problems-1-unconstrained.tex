...
\lecturechapter{Optimization Problems: \\Unconstrained problems}
...
\begin{footnotesize}
\begin{eqnarray*}
	\min_{\thetab \in \Theta} \sumin \left(-\yi \cdot \thetab^\top\xi + \log(1 + \exp\left(\thetab^\top\xi\right)\right) + \lambda \left[\frac{1-\alpha}{2}\|\theta\|^{2}_{2} + \alpha\|\theta\|_{1}\right], \\ \lambda \geq 0, \alpha \in [0,1]
\end{eqnarray*}
\end{footnotesize}
...
\vspace*{0.5cm}
\begin{figure}
	\includegraphics{figure_man/logreg-0.pdf}
\end{figure}
\end{column}
...
\begin{column}{.3\textwidth}
\begin{figure}
	\includegraphics{figure_man/logreg-5-1.pdf}	
\end{figure}
\end{column}
\end{columns}
...
\begin{itemize}
	\item Multivariate
	\item Unconstrained
	\item Smooth for $\alpha = 0$, not smooth for $\alpha > 0$
	\item Convex
\end{itemize}
