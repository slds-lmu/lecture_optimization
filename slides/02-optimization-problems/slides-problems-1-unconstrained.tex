...
\end{itemize}
\comment{BB}{komisch dass wir das hier mit S bringen wenn unconstrained?}
\end{framev}
...
\comment{BB}{unglücklich in der notatioon hier f für die densitity zu  nehmen.... f war unsere objective....}
\end{framei}
...
\comment{BB}{wir sollten den loss plotten, so kann man ihn besser erklären}
\end{framei}
...
\comment{BB}{wurde das wirklich mit den konstanten so implementiert im r code?}

\begin{framei}{Example 4: (Regularized) Log. Regression}
\comment{BB}{vielleicht logreg auch lieber mit +1-1 definieren? und die ähnlichkeit zwiwschen SVM und LR zeigen?}
...
\end{small}
\comment{BB}{nicht ganz klar warum das konvesx bleibt? mit dem K_i}
\end{framei}
...
\comment{BB}{1-2 mehr formeln hin, zb für neuron und activation function, damit man besser die struktur von f / NN versteht. auch bild von sigmoid und RELU activ fun hin}
\end{framei}
