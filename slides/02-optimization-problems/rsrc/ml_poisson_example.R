...
loglik = sapply(lambda, likelihood)
df = data.frame(lambda = lambda, loglik = loglik)

optimum = optimize(likelihood, interval = c(0, 1), maximum = TRUE)

p = ggplot() + geom_line(data = df, aes(x = lambda, y = loglik)) + geom_point(data = data.frame(x = optimum$maximum, y = optimum$objective), aes(x = x, y = y), colour = "blue") + xlab(expression(lambda))
