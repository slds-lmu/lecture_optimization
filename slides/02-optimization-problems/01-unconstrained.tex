...
% FIXME: this chapter is partially bullshit. we should distribute these problem definitions to the chapters where they are actually used
...
\begin{framev}{Unconstrained optimization problem}
$$
...
\begin{itemize}
\item \textbf{unconstrained}, if $\mathcal{S} = \R^d $
\item \textbf{smooth} if $f$ is at least $\in \mathcal{C}^1$
\item \textbf{univariate} if $d = 1$, and \textbf{multivariate} if $d > 1$
\item \textbf{convex} if $f$ convex function (on convex $\R^d$)
\end{itemize}
\comment[NOTE]{BB}{komisch dass wir das hier mit S bringen wenn unconstrained?}
\end{framev}


\begin{framei}[sep=L]{Note: A Convention in Optimization}
\item W.l.o.g., we always \textbf{minimize} functions $f$.
\item Maximization is handled by minimizing $-f$
\vfill
\splitV[0.5]
{
\imageC[1]{figure/ml_poisson_example_1.pdf}
}
{
\imageC[1]{figure/ml_poisson_example_2.pdf}
}
\end{framei}
...
\imageC[0.5]{figure/ml_normal_example_dnorm.pdf}
\comment[NOTE]{BB}{unglücklich in der notatioon hier f für die densitity zu  nehmen.... f war unsere objective....}
\end{framei}
...
\imageC[0.4]{figure/ml_normal_example_negloglike_nooptim.pdf}
\end{framei}


\begin{framei}{Example 1: Maximum Likelihood}
\item[] $$ \min_{\mu \in \R} - \ell(\mu, \D). $$
\item can be solved analytically (setting the first deriv. to $0$) since it is a quadratic function:
$$ -\frac{\partial \ell(\mu, \D)}{\partial \mu} = \sumin \left(\xi - \mu\right) = 0 \quad \Leftrightarrow \quad \hat \mu = \frac{1}{n} \sumin \xi $$
\imageC[0.4]{figure/ml_normal_example_negloglike.pdf}
\end{framei}


\begin{framei}[sep=L]{Example 1: Maximum Likelihood}
\item Was: \textbf{smooth}, \textbf{univariate}, \textbf{unconstrained}, \textbf{convex}
\item If we had optimized for $\sigma$ as well (instead of assuming it as fixed)
$$ \min_{\mu \in \R, \sigma \in \R^+} - \ell(\mu, \D) $$
\item The problem would have been bivariate and constrained
\end{framei}
...
\vfill
\imageC[0.6]{figure/ml_linreg_example_1.png}
\comment[TODO]{UU}{Add proof of convexity of LS}
\end{framei}
...
\vfill
\splitV{
\imageC[0.9]{figure/ml_linreg_example_1.png}
}{
\imageC[1]{figure/ml_linreg_example_2.png}
}
...
\comment[NOTE]{BB}{wir sollten den loss plotten, so kann man ihn besser erklären}
\end{framei}
...
\comment[NOTE]{BB}{wurde das wirklich mit den konstanten so implementiert im r code?}

\begin{framei}{Example 4: (Regularized) Log. Regression}
\comment[NOTE]{BB}{vielleicht logreg auch lieber mit +1-1 definieren? und die ähnlichkeit zwiwschen SVM und LR zeigen?}
...
\end{small}
\comment[NOTE]{BB}{nicht ganz klar warum das konvesx bleibt? mit dem K_i}
\end{framei}
...
\comment[NOTE]{BB}{1-2 mehr formeln hin, zb für neuron und activation function, damit man besser die struktur von f / NN versteht. auch bild von sigmoid und RELU activ fun hin}
\end{framei}
