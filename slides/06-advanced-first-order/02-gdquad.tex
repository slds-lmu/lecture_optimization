...
\comment[NOTE]{BB}{es gibt eine analyse von GD auf GF als dyn system. das sagt uns nette stabile lösungen. ich weiß nicht ob das zu schwer ist?
https://ieeexplore.ieee.org/document/8706530}
\comment[NOTE]{BB}{über conjug grad mal reden? mirror GD?}
...
\vspace{0.5cm}
\item[] \hspace{-0.7cm}Slides based on \furtherreading{DISTILL2017MOMENTUM}
}

\begin{framei}[align=top]{Quadratic forms \& GD}
...
$$
\imageFixed{4.5cm}{5cm}[0.65]{figure/gd_quadratic_forms_xspace.pdf}
\end{framei}


\begin{framei}[align=top]{Eigendecomposition of quadratic forms}
...
\imageFixed{4.5cm}{5cm}[0.65]{figure/gd_quadratic_forms_wspace.pdf}
\end{framei}
...
\begin{framev}{GD steps in eigenspace}
...
\item The smaller $|1 - 2\alpha\lambda_i|$ is, the faster component $i$ decays; values close to $1$ mean almost no progress per step
\item Eigenvectors with larger eigenvalues typically converge fastest, especially early on
\end{framei}


\begin{framei}{GD regret}
\item We can also analyze the regret (in objective value space):
$$
...
\item Rates become more similar once the slow modes (eigenvector components with small $\lambda_i$) dominate
\vfill
\imageC[0.45]{figure/gd_conv.pdf}
\end{framei}
...
\comment[NOTE]{LS}{we have xTAx -bTx the blogpost has 1/2 and in the next slideset sometimes a 2 s missing;
Visualisierung;
alpha <= 1/(2 lambda_min);
symmetric}
