...
\footnotesize
  ...
  \State{Compute predictions for all points in the design $\hat{f}(\xv^{(1)}), \ldots, \hat{f}(\xv^{(t)})$}
  \State{Train an interpolating GP on $\left\{\left(\xv^{(1)}, \hat{f}(\xv^{(1)})\right), \ldots, \left(\xv^{(t)}, \hat{f}(\xv^{(t)})\right)\right\}$}
