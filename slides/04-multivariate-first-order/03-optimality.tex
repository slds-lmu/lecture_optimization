...
\comment[NOTE]{LS}{check the following https://arxiv.org/abs/1405.4980
exponential convergence https://www.cs.ubc.ca/~schmidtm/Courses/540-W19/L5.pdf
exponential rate of convergence zeigen?
mu-strongly convexity?}
\begin{itemizeM}
\item $f$ convex and differentiable
\item Global minimum $\xvs$ exists
\item $f$ has Lipschitz gradient ($\nabla f$ does not change too fast)
\begin{framed}
$$\| \nabla \fx - \nabla f(\xtil) \| \le L \|\xv - \xtil\| \quad \text{ for all $\xv, \xtil$}$$
\end{framed}
\end{itemizeM}
\begin{framed}
\textbf{Theorem \textnormal{(Convergence of GD)}.}
GD with step size $\alpha \leq 1/L$ yields
$$f(\xv^{[k]}) - f(\xvs) \leq \frac{\| \xv^{[0]} - \xvs \|^2}{2\alpha k}$$
In other words: GD converges with rate $\mathcal{O}(1/k)$.
\end{framed}
\end{framei}


\begin{framev}{Proof strategy}
\begin{enumerate}
...
\item[] \begin{align*}
f(\xv^{[t+1]}) &\leq f(\xv^{[t]}) + \nabla f(\xv^{[t]})^T(\xv^{[t+1]} - \xv^{[t]}) + \frac{L}{2} \|\xv^{[t+1]} - \xv^{[t]}\|^2 \\
&= f(\xv^{[t]}) + \nabla f(\xv^{[t]})^T(\xv^{[t]} - \alpha \nabla f(\xv^{[t]}) - \xv^{[t]}) \\
...
$$f(\xv^{[t]}) - f(\xvs) \leq  \nabla f(\xv^{[t]})^T (\xv^{[t]} - \xvs).$$
\item This and the descent lemma yields
\begin{align*}
f(\xv^{[t+1]}) - f(\xvs) &\leq f(\xv^{[t]})  - \frac{\alpha}{2}\|\nabla f(\xv^{[t]})\|^2  - f(\xvs) \\
&= f(\xv^{[t]})  - f(\xvs)  - \frac{\alpha}{2}\|\nabla f(\xv^{[t]})\|^2 \\
&\leq \nabla f(\xv^{[t]})^T (\xv^{[t]}-\xvs) - \frac{\alpha}{2}\|\nabla f(\xv^{[t]})\|^2 \\
&= \frac{1}{2 \alpha} \left( \|\xv^{[t]}-\xvs\|^2 - \| \xv^{[t]} - \xvs - \alpha \nabla f(\xv^{[t]})\|^2 \right) \\
&= \frac{1}{2 \alpha} \left( \|\xv^{[t]}-\xvs\|^2 - \| \xv^{[t+1]} - \xvs \|^2 \right)
\end{align*}
...
\begin{align*}
k ( f(\xv^{[k]}) - f(\xvs)) &\leq \sum_{t=1}^{k} [f(\xv^{[t]}) - f(\xvs)]\\
&\leq \sum_{t=1}^{k} \frac{1}{2 \alpha} \left[ \|\xv^{[t-1]}-\xvs\|^2 - \| \xv^{[t]} - \xvs \|^2 \right] \\
&= \frac{1}{2 \alpha}  \left( \|\xv^{[0]}-\xvs\|^2 - \| \xv^{[k]} - \xvs \|^2 \right) \\
&\leq \frac{1}{2 \alpha} \left( \|\xv^{[0]}-\xvs\|^2 \right).
\end{align*}
\item\textbf{Arguments:} Descent lemma (line~1).\\ Telescoping sum (line~2 $\to$ 3).
\begin{framed}
$$f(\xv^{[t+1]}) - f(\xvs) \leq \frac{\| \xv^{[0]} - \xvs \|^2}{2\alpha k}$$
\end{framed}
\end{framei}
