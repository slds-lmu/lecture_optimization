...
\begin{itemize}
    ...
    \item For logistic regression, set $\bm{Y} \sim \mathcal{B}(\bm{\pi}), \bm{\pi} = \frac{1}{1+e^{-\bm{X}\thetav^{\ast}}}$
    ...
    \item ERM has unique global minimizer given by $\thetah = (\bm{X}^{\top}\bm{X})^{-1}\bm{X}^{\top} \bm{Y}$ (linear)
