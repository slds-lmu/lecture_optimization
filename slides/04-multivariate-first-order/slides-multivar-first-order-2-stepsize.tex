...
\comment[NOTE]{BB}{Bernd: eigentich ist alles ganz schön hier, 2 sachen könnte man noch besser machen a) man kann nicht wirklich sehen, welche stepsize hier verwendet wird, links und mitte ist das ja nur eine konstante, aber rechts soll es ja "adaptive" sein, vielleicht kann man das besser zeigen. b) generell könnte vielleicht 4 fälle zeigen. A. zu gross B. zu klein. C. gute konstante D. adaptive}
...
Steps of line searches for $f(\xv) = 10 x_1^2 + x_2^2/2$
\end{footnotesize}\end{center}
\end{framei}


\begin{framei}{Step size control: Diminishing step size}
\comment[NOTE]{BB}{Bernd: auch alles tiotal ok so, aber an sich kann man fast überlegen diese beiden beispiele zusammenzulegen.... und dann fände ich eigentlich immer 2d besser als 1d. die zeigen fast das gleiche. passt aber auch so}
...
\comment[NOTE]{BB}{Bernd: eigentlich könnte man das mininmal mehr ausführen. 1. die schrittweitensuche kostet arbeit 2. es ist bnicht mal klar, ob man bei optimaler schrittweite WENIGER schritte ins optimum macht, gerade bei GD und schlechter conditio sollte sich das GEGENTEIL zeigen lassen: nehmen wir die optimale schritteweite machen wir eher viele schritte (?)}
