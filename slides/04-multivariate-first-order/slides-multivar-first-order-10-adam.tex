...
\comment[NOTE]{BB}{polyak noch rein irgendwo}
...
\item Understand the idea of adaptive step size methods
\item Understand the adaptive step size methods AdaGrad, RMSProp, and Adam and their differences
}

\begin{framei}{Adaptive step sizes}
\item Besides descent direction, step size is the other important control parameter in first order methods with strong influence on performance
\item Motivated by limitations of global step sizes in poorly-conditioned problems, it is natural to use different step size for each dimension individually and automatically adapt sizes over time
\vfill
...
\end{itemize}
\item This effectively changes the direction of the update (and not just the magnitude) by scaling the gradient element-wise
% \item \textbf{But:} Accumulation of squared gradients can result in premature small step sizes %\furtherreading{goodfellow2016deep}
\begin{algorithm}[H]
\footnotesize
\caption{AdaGrad}
\begin{algorithmic}[1]
\State \textbf{require} Initial parameter $\thetav$, \strut Global step size $\alpha$, \strut Small constant $\beta$ for numerical stability (e.g. $10^{-7}$) \strut 
...
\item AdaGrad's accumulation of squared gradients via summation can result in radically diminishing step sizes
\item RMSProp is a modification of AdaGrad that resolves this by using an exponentially weighted moving average of past squared gradients
\item Theoretically, RMSProp leads to performance gains in non-convex scenarios. Empirically, it is a very effective optimization algorithm routinely employed by DL practitioners. 
\vspace{-0.5em}\begin{algorithm}[H]
\footnotesize
\caption{RMSProp}
\begin{algorithmic}[1]
\State \textbf{require} Initial parameter $\mathbf{\thetav}$, \strut Global step size $\alpha$, \strut Small constant $\beta$ for numerical stability (e.g. $10^{-7}$), \strut decay rate $\rho \in [0, 1)$
\State \textbf{Initialize} gradient accumulation variable $\mathbf{r} = \mathbf{0} $
\While{stopping criterion not met}
\State \parbox[t]{\dimexpr\linewidth-\algorithmicindent}{Sample minibatch of $m$ examples from the training set $\{\tilde{\xv}^{(1)},\dots,\tilde{\xv}^{(m)}\}$ \strut}
...
\begin{framei}{Adam}
\item Adaptive Moment Estimation (ADAM) uses the 1st and 2nd moments of gradients
\begin{itemize}
    \item 1st moment: Exponentially decaying average of past gradients as momentum term
    \item 2nd moment: Exponentially decaying average of past squared gradients to adapt step sizes like RMSProp
\end{itemize}
\item Can thus be seen as combination of RMSProp and momentum
\end{framei}
...
\caption{Adam}
\begin{algorithmic}[1]
\State \textbf{require} Initial parameters $\thetav$, \strut Global step size $\alpha$ (suggested default: 0.001), \strut Small constant $\beta$ (suggested default $10^{-8}$), \strut Exponential decay rates for moment estimates $\rho_1$ and $\rho_2$ in $[0,1)$ (suggested defaults: 0.9 and 0.999 respectively) \strut 
\State \textbf{Initialize} time step $t = 0$
\State \textbf{Initialize} 1st and 2nd moment variables $\mathbf{s}^{[0]} = 0, \mathbf{r}^{[0]} = 0$
\While{stopping criterion not met}
...
\State \parbox[t]{\dimexpr\linewidth-\algorithmicindent}{Compute update: $\nabla\thetav = - \frac{\alpha}{\beta +\sqrt{\hat{\mathbf{r}}}} \odot \hat{\mathbf{s}}$ \strut}
\State Apply update: $\thetav \leftarrow \thetav + \nabla\thetav$
\EndWhile
\end{algorithmic}
\end{algorithm}
...
\item \textbf{Note:} Contributions of past $\hat{\mathbf{g}}^{[i]}$ decrease rapidly
\end{framei}
...
The resulting error is put in $\zeta$ and is kept small due to the exponential weights of past gradients
...
\vfill
\textbf{Comparison} of SGD variants near saddle point \\
...
\comment[NOTE]{BB}{nicht so toll weil adam hier gar nicht dabei ist, und andere die wir nicht kennen. außerdem wären mehere screenshorts scvhön damit man die anim direkt auf den folien zeigen kann}
\end{framev}


\begin{framev}{Comparison on quadratic form}
\image{figure_man/momentum/comparison_adam.png}
\comment[NOTE]{UU}{Kann der Plot überhaupt stimmen?  Woher weiß Adam gleich am Startpunkt, dass es in Richtung Minimum marschiert?}
