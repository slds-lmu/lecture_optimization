...
plot_combined_losses <- function(loss_histories, test_loss_histories, l2_diff_histories, 
  ...
  method_colors <- c(
    "GD" = "#B3E19B",
    "GD+mom" = "#67A8CD",
    "GD+mom+ExpDecay" = "#E67E26",
    "SGD" = "#FFE659",
    "SGD+mom" = "#FF9D9F",
    "SGD+mom+ExpDecay" = "#9A7FBD"
  )
  ...
  line_types <- c("GD" = "longdash",
                  "GD+mom" = "dashed",
                  "GD+mom+ExpDecay" = "dotted",
                  "SGD" = "solid",
                  "SGD+mom" = "solid",
                  "SGD+mom+ExpDecay" = "solid")
  
  # Plot Training Loss
  p1 <- ggplot(df_loss, aes(x = Iteration, y = Loss, color = Method, linetype = Method)) +
  ...
  # Plot Test Loss
  p2 <- ggplot(df_test_loss, aes(x = Iteration, y = Loss, color = Method, linetype = Method)) +
  ...
  # Plot L2 Difference
  p3 <- ggplot(df_l2_diff, aes(x = Iteration, y = Error, color = Method, linetype = Method)) +
...
plot_coef_path_iter <- function(beta_history, method, beta_true, 
                                  show_legend=FALSE, n_beta=10) {
  p <- ncol(beta_history)
  if(n_beta > p) {
    stop("The number of paths should not exceed the number of coefficients.")
  }
  path <- beta_history[,1:n_beta]
  beta_true <- beta_true[1:n_beta]
  # draw only path of first n_beta coefficients
  
  colnames(path) <- paste0("θ", seq_len(n_beta))
  path_df <- as.data.frame(path)
  path_df$Iteration <- 1:nrow(path)
  path_long <- melt(path_df, id.vars = "Iteration")
  
  beta_true_df <- data.frame(
    Iteration = max(path_df$Iteration),
    value = beta_true,
    variable = colnames(path)
  )
  
  p_path <- ggplot(path_long, aes(x = Iteration, y = value, color = variable)) +
    geom_line() +
    geom_point(data = beta_true_df, aes(x = Iteration, y = value, color = variable), shape = 18, size = 2) +
    labs(title = method,
         x = "Iteration", y = "Coefficient Value") +
    theme_minimal()
...
plot_coef_path_time <- function(beta_history, time_history, method, beta_true, 
                                  show_legend=FALSE, n_beta=10) {
  p <- ncol(beta_history)
  if(n_beta > p) {
    stop("The number of paths should not exceed the number of coefficients.")
  ...
  max_t = max(time_history)
  
  colnames(path) <- paste0("θ", seq_len(n_beta))
  path_df <- as.data.frame(path)
  path_df$Time <- time_history
  path_long <- melt(path_df, id.vars = "Time")
  ...
  x_ticks <- round(seq(0, max_t, length.out = 4),5)
  
  p_path <- ggplot(path_long, aes(x = Time, y = value, color = variable)) +
    geom_line() +
    geom_point(data = beta_true_df, aes(x = Time, y = value, color = variable), shape = 18, size = 2) +
    labs(title = method,
         x = "Time (sec)", y = "Coefficient Value") +
    scale_x_continuous(breaks = x_ticks, limits = c(0, max_t)) +
    theme_minimal()
...
beta_histories[[1]] <- result_gd$beta_history
time_histories[[1]] <- result_gd$time_history
method_names[1] <- "GD"
...
beta_histories[[2]] <- result_gd_momentum$beta_history
time_histories[[2]] <- result_gd_momentum$time_history
...
beta_histories[[3]] <- result_gd_momentum_decay$beta_history
time_histories[[3]] <- result_gd_momentum_decay$time_history
...
beta_histories[[4]] <- result_sgd$beta_history
time_histories[[4]] <- result_sgd$time_history
method_names[4] <- "SGD"
...
beta_histories[[5]] <- result_sgd_momentum$beta_history
time_histories[[5]] <- result_sgd_momentum$time_history
...
beta_histories[[6]] <- result_sgd_momentum_decay$beta_history
time_histories[[6]] <- result_sgd_momentum_decay$time_history
...
path_i_plots <- list()
for(i in 1:length(beta_histories)) {
  if (i %% 3 == 0) {
    path_i_plots[[i]] <- plot_coef_path_iter(beta_histories[[i]],method_names[i],beta_true,show_legend=TRUE)
  } else {
    path_i_plots[[i]] <- plot_coef_path_iter(beta_histories[[i]],method_names[i],beta_true)
    }
}
p_path_i <- grid.arrange(grobs = path_i_plots, ncol = 3, nrow = 2, widths = c(1, 1, 1.1))
ggsave(filename = "figures/combined_path_iter.png", 
       plot = p_path_i, 
       width = 16, 
       height = 6,
       dpi = 300)
...
path_t_plots <- list()
#widths <- sapply(time_histories, max)
for(i in 1:length(beta_histories)) {
  if (i %% 3 == 0) {
    path_t_plots[[i]] <- plot_coef_path_time(beta_histories[[i]],time_histories[[i]],method_names[i],beta_true,show_legend=TRUE)
    #widths[i] <- widths[i]*1.1 # legend
  } else {
    path_t_plots[[i]] <- plot_coef_path_time(beta_histories[[i]],time_histories[[i]],method_names[i],beta_true)
  }
}

# Arrange each row individually with specific width ratios
# row1 <- grid.arrange(grobs=head(path_t_plots, 3), ncol = 3, widths = widths[1:3])
# row2 <- grid.arrange(grobs=tail(path_t_plots, 3), ncol = 3, widths = widths[4:6])
# p_path_t <- grid.arrange(row1, row2, ncol = 1, heights = c(1,1))
p_path_t <- grid.arrange(grobs=path_t_plots, ncol = 3, nrow=2, widths = c(1,1,1.1))
ggsave(filename = "figures/combined_path_time.png", 
       plot = p_path_t, 
       width = 16, 
       height = 6,
       dpi = 300)
