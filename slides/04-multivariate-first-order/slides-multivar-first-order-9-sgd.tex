...
\item Understand effect of batch size and step size
\item Understand stopping rules
}
...
\end{eqnarray*}
with random subset $J \subset \{1, 2, \ldots, n\}$ %of gradients 
called \textbf{mini-batch}
\item This is done e.g. when computing the true gradient is \textbf{expensive}
\item SGD is a \textbf{stochastic optimization} method as it uses stochastic gradient estimates
\end{framei}

\begin{framev}{SGD algorithm}
\begin{algorithm}[H]
\footnotesize
...
\only<1>{\small (Note that each SGD step for $m = 1$ corresponds to an exact GD step on one of the $g_i$.)}
\only<5>{\small In iteration $5$, SGD performs a suboptimal move away from the minimum:}
\only<1>{\imageFixed{0.5cm}{2.4cm}[0.95]{figure_man/sgd_example_iter_1.png}}
\only<2>{\imageFixed{0.5cm}{2cm}[0.95]{figure_man/sgd_example_iter_2.png}}
\only<3>{\imageFixed{0.5cm}{2cm}[0.95]{figure_man/sgd_example_iter_3.png}}
\only<4>{\imageFixed{0.5cm}{2cm}[0.95]{figure_man/sgd_example_iter_4.png}}
\only<5>{\imageFixed{0.5cm}{2cm}[0.95]{figure_man/sgd_example_iter_5.png}}
\end{frame}
...
\end{itemize}
\item Consequently, SGD has worse convergence properties than GD
\item \textbf{But:} Can be controlled by tuning \textbf{batch size} and \textbf{step size}.
\end{framei}

\begin{framei}{Effect of batch size}
\item The larger the batch size $m$
\begin{itemize}
    ...
    \item \textbf{But:} Maximum batch size is usually limited by computational resources (memory)
\end{itemize}
\vfill
\imageC[0.9]{figure_man/sgd_example_batch_size.png}
\begin{center}\begin{small}
Example continued: Batch size $m = 1$ vs. $m = 5$
\end{small} \end{center}
\end{framei}

\comment[NOTE]{BB}{wir könnten noch etwas über die verteilung von den grad reden, CLT usw. können wir später}
\comment[NOTE]{BB}{hier könnte man die claims als bsp zeigen
vielleicht kann man mal batchsize und step size variieren auf einem praktiven bsp}
\comment[NOTE]{BB}{wäre schön wenn wir die aussagen hier empirisch besser zeigen würden
Hier könnte man vielleicht fündig werden: https://udlbook.github.io/udlbook/ Kap. 9+20.}
\comment[NOTE]{BB}{was hier auf der folie steht ist nicht so super klar finde ich.... können wir das etwas verbessern}
\comment[NOTE]{UU}{Man könnte noch einen Bezug herstellen zwischen der Größe dieser Varianz und der Größe der "confusion area".}
\comment[NOTE]{BB}{es wäre schön einmal SGD gegen GD auf echten daten zu sehen. zb bei einem NN. man muss gucken wie man die x-achse skaliert. man kann einmal fevals vs y zeigen. aber auch "datenpunkt" gegen y.
wir wollen vielleicht auch darüber reden dass SGD saddle points escaped kann}

\begin{framei}{Effect of step size}
\item The smaller the step size $\alpha$
\begin{itemize}
    \item the smaller erratic steps in potentially wrong directions
    \item the lower the effect of gradient variance
    \item \textbf{But:} Risk of impaired performance by not moving far enough
\end{itemize} 
...
\begin{framei}{Step size control for SGD}
\item Popular solution: Decreasing step size %fulfilling $\alpha^{[t]} \in \order(1/t)$
\begin{itemize}
    \item Initially, step size shall be large enough to make progress
    \item In later iterations, step size shall be small enough to reduce effect of gradient variance
\end{itemize}
\vfill
...
\end{small} \end{center}
\vfill
% \item (Common alternative: $\alpha^{[t]} \in \order(1/\sqrt{t})$ to avoid step size getting small too fast)
\end{framei}

\begin{framev}{Step size control for SGD}
...
\item Research aims at finding inexact line search methods that provide better convergence behavior, e.g., \furtherreading{vaswani2019painless}
% Vaswani et al., \emph{Painless Stochastic Gradient: Interpolation, Line-Search, and Convergence Rates}. NeurIPS, 2019
\end{blocki}
\end{framev}
...
\begin{framev}[fs=small]{SGD and ML}
\begin{blocki}{General remarks:}
\item SGD is a variant of GD
\item SGD particularly suitable for large-scale ML when evaluating gradient is too expensive
\item Thus, SGD and variants are the most commonly used methods in modern ML with e.g. neural networks
\item Note that even for the linear model and quadratic loss, where a closed form solution is available, SGD is often used if the size $n$ of the dataset is too large and the design matrix does not fit into memory
\end{blocki}
\end{framev}

\comment[NOTE]{BB}{wir wollen die regu eigenschaften von sgd besser erkären
