...
\textbf{Red area} (\enquote{confusion area}): $-\nabla g_i(\xv)$ might point away from minimum of $g(\xv)$ and perform a suboptimal move
...
\end{itemize}
\item As maximum batch size is usually limited by computational resources (memory), choosing a suitable step size is crucial
\vfill
...
\begin{itemize}
    \item the smaller a step in a potentially wrong direction (reduced magnitude of erratic steps)
...
\begin{framev}{Advanced step size control}
