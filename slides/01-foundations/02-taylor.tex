...
\splitVCC[0.4]
{\imageC{figure/taylor_univariate.png}}
{\image[0.7]{figure_man/Taylor2D_2nd-100.png}}
\end{framei}
...
\begin{framev}{Taylor polynomials}
\begin{itemizeM}
...
\begin{framev}{Multivariate Taylor polynomials}
For the multivariate version, we need a concise way to express derivatives and powers involving several variables
\begin{itemizeM}
\item A \textbf{multi-index} is a vector $\bm{\alpha} = (\alpha_1, \alpha_2, \dots, \alpha_d) \in \mathbb{N}^d.$
\item Its \textbf{order} is the sum of its components: $|\bm{\alpha}| = \alpha_1 + \alpha_2 + \cdots + \alpha_d.$
\item Partial derivative is written as $D^{\bm{\alpha}} f = \dfrac{\partial^{|\bm{\alpha}|} f}{\partial x_1^{\alpha_1} \cdots \partial x_d^{\alpha_d}}$
\item Factorials generalize componentwise: $\bm{\alpha}! = \alpha_1! \, \alpha_2! \cdots \alpha_d!.$
\item For $\xv,\,\bm{a}\in\R^d$: $(\xv - \bm{a})^{\bm{\alpha}} = (x_1 - a_1)^{\alpha_1} \cdots (x_d - a_d)^{\alpha_d}.$
\end{itemizeM}
...
\begin{framev}{Multivariate Taylor polynomial identities}
For $f \in \mathcal{C}^k(I,\R)$ as before, we will often use the following identities:
\begin{itemizeM}
...
\begin{framei}[fs=footnotesize]{Specific remainder forms}
\item Beyond the $\littleo$ bound, two exact forms are workhorses in convergence proofs:
\item[] \textbf{Integral form ($f \in \CC{1}$):}
$$
f(\xv+\bm{h}) = f(\xv) + \int_0^1 \nabla f(\xv + \gamma \bm{h})\, \bm{h}\; d\gamma
$$
\item[] \textbf{Mean-value form ($f \in \CC{1}$):}
$$
f(\xv+\bm{h}) = f(\xv) + \nabla f(\xv + \gamma \bm{h})\, \bm{h} \qquad \text{for some } \gamma \in (0,1)
$$
\item[] \textbf{Second-order mean-value form ($f \in \CC{2}$):}
$$
f(\xv+\bm{h}) = f(\xv) + \nabla f(\xv)\bm{h} + \tfrac{1}{2}\bm{h}^T \nabla^2 f(\xv + \gamma \bm{h}) \bm{h} \qquad \text{for some } \gamma \in (0,1)
$$
\item Used later, e.g., to prove the 2nd-order sufficient condition and the descent lemma
\end{framei}
...
\begin{framev}{Examples of analytic functions}
...
\begin{framev}{Example: Taylor Approximation of $\fx = \sin(2\MakeLowercase{x}_1) + \cos(\MakeLowercase{x}_2)$ at $\bm{\MakeLowercase{a}} = (1, 1)^T$}
\,\\
\begin{footnotesize}
\textbf{1st order}: we know that $f(\xv) = \underbrace{f(\bm{a}) + \nabla f(\bm{a}) (\xv - \bm{a})}_{T_1(\xv, \bm{a})} + R_1(\xv,\bm{a})$
...
\splitVCC{
\imageC[0.7]{figure_man/Taylor2D_1st100.png}}
{\imageC[0.8]{figure_man/Taylor2D_1st301.png}}
\end{framev}
...
\vspace*{-0.5\baselineskip}
\splitVCC{
\imageC[0.7]{figure_man/Taylor2D_2nd-100.png}
}{\imageC[0.7]{figure_man/Taylor2D_2nd-301.png}
}
\end{framev}
