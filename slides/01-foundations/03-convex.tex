...
$$ f(\xv + t(\yv - \xv)) \le f(\xv) + t(f(\yv) - f(\xv)) $$
\item Intuition: Connecting line for any $\xv, \yv \in \S$ above function $f$
\splitV{
\imageC[0.48]{figure/convexity_1.pdf}
}{
\imageC[0.48]{figure/convexity_2.pdf}
}
...
\imageC[0.45]{figure/conv_conc_functions.png}
\end{framei}
...
\imageC[0.4]{figure/conv-first-order-cond.png}
\item Strictly convex if $>$ instead of $\ge$
\end{framei}


\begin{framei}{Second order condition}
\item For $f \in \CC{2}$: can characterize convexity via Hessian
\item $f$ convex $\Longleftrightarrow$ $H(\xv)$ psd $\forall \xv \in \S$
...
or, equivalently, check that all eigenvalues $\lambda_i$ of all $H(\xv)$ satisfy $\lambda_i \geq 0$ for all $\xv \in \S$
\end{framei}
...
\item $f$ is convex since $H(\xv)$ is p.s.d. for all $\xv\in\S$:
\begin{align*}
...
\begin{framei}{First-order optimality (convex case)}
\item For convex differentiable $f$: vanishing gradient is sufficient for a global min
\item \textbf{Theorem.} Let $f$ be continuously differentiable and convex.\\
If $\nabla f(\xvs) = 0$, then $\xvs$ is a global minimizer.
\item \textbf{Proof.} Set $\xv = \xvs$ in the first-order characterization:
$$
f(\yv) \geq f(\xvs) + \nabla f(\xvs)(\yv - \xvs) = f(\xvs) \qquad \forall \yv \in \S
$$
\item Hence for convex $f$: first-order necessary $\Leftrightarrow$ first-order sufficient
\end{framei}


\begin{framei}{Strong convexity}
\item $f$ is \textbf{strongly convex} with modulus $m > 0$ if $\forall \xv, \yv \in \S$, $t \in [0,1]$:
$$
f(\xv + t(\yv-\xv)) \leq f(\xv) + t(f(\yv) - f(\xv)) - \tfrac12 m\, t(1-t)\|\xv-\yv\|^2
$$
\item Stronger than convex: extra negative term $-\tfrac12 m\, t(1-t)\|\xv-\yv\|^2$ on the right
\item For differentiable $f$, equivalently:
$$
f(\yv) \geq f(\xv) + \nabla f(\xv)(\yv-\xv) + \tfrac{m}{2}\|\yv-\xv\|^2
$$
\item \textbf{Interpretation:} $f$ is lower-bounded by a quadratic touching $f$ at $\xv$\\
(complements the upper bound from $L$-smoothness; cf. descent lemma)
\end{framei}


\begin{framei}{Strong convexity: Hessian and uniqueness}
\item For $f \in \CC{2}$: $f$ is $m$-strongly convex $\Longleftrightarrow$ $H(\xv) \succeq m \mathbf{I}_d$ $\forall \xv \in \S$
\item Equivalently, all eigenvalues of $H(\xv)$ are bounded below by $m > 0$
\item Combined with $L$-smooth $\Leftrightarrow$ $H(\xv) \preceq L \mathbf{I}_d$:
$$
m \mathbf{I}_d \preceq H(\xv) \preceq L \mathbf{I}_d
$$
\item Ratio $\kappa = L/m$ is the \textbf{condition number} of $f$ (later: governs convergence rate)
\spacer
\item \textbf{Theorem.} For strongly convex $f$, if $\nabla f(\xvs) = 0$, then $\xvs$ is the \textbf{unique} global minimizer.
\item \textbf{Proof.} From the gradient form with $\xv = \xvs$ and $\nabla f(\xvs) = 0$:
$$
f(\yv) \geq f(\xvs) + \tfrac{m}{2}\|\yv - \xvs\|^2 > f(\xvs) \qquad \forall \yv \neq \xvs
$$
\end{framei}
