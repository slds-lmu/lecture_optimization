...
\begin{framev}[align=top, fs=small]{CMA-ES: Basic Method - Iteration 1}
\begin{enumerate} \setcounter{enumi}{-1}
\item Initialize $\mathbf{m}^{[0]},\sigma^{[0]}$ problem-dependent and $\mathbf{C}^{[0]}=\mathbf{I}_{d}$
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_1.png}
\end{framev}
...
$$
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_2.png}
\end{framev}
...
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_21.png}
\end{framev}
...
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_3.png}
\end{framev}


\begin{framev}[align=top, fs=small]{CMA-ES: Basic Method - Iteration 1}
\begin{enumerate} \setcounter{enumi}{2}
\item \textbf{Update covariance matrix} Roughly: elongate density ellipsoid in direction of successful steps.
\begin{itemize} \begin{small}
\item $\mathbf{C}^{[1]}$ reproduces successful points with higher probability than $\mathbf{C}^{[0]}$.
\item Update $\mathbf{C}^{[0]}$ using sum of outer products and parameter $c_{\mu}$:
$\mathbf{C}^{[1]} = (1-c_{\mu}) \mathbf{C}^{[0]} + c_{\mu} \sum_{i=1}^{\mu} w_i \yv_{i:\lambda}^{[1]}(\yv_{i:\lambda}^{[1]})^T$ (rank-$\mu$ update).
\end{small} \end{itemize}
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_4.png}
\end{framev}


\begin{framev}[align=top, fs=small]{CMA-ES: Basic Method - Iteration 2}
\begin{enumerate}
\item \textbf{Sample} from distribution for new generation
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_5.png}
\end{framev}
...
\item \textbf{Update mean}
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_6.png}
\end{framev}
...
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_7.png}
\end{framev}
...
\end{itemize}
\end{enumerate}
\imageFixed{2cm}{3.6cm}[0.9]{figure_man/cmaes/cmaes_update_8.png}
\end{framev}
...
\comment[NOTE]{BB}{ich vermute dass hier die w wieder fast immer 0/1 sind; nein wir nehmen immer w=1 weil wir mu auswählen}
\comment[NOTE]{BB}{folie nicht so schön. sehr viel redundanz aber wenig erklärt wie das rank1 update geht. das sigma von oben kommt gar nicht mehr vor.}
\comment[NOTE]{CK}{in tutorial werden rank-1 updates entweder mit dem best-performing candidate y_{1:lambda} durchgeführt oder mit dem evolution path, aber afaik nicht mit einer "auxiliary variable" wie hier.}
\comment[NOTE]{CK}{das update kommt im tutorial afaik nicht vor}
\comment[NOTE]{BB}{ziemlich unkllar was auf dem bild hier passiert, warum sind es 4 kurven aber nur 3 in der legende? außerdem wird sehr wenig erklärt}
