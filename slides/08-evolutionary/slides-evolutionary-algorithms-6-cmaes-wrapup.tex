...
\State $\yv_w = \sum_{i=1}^{\mu} w_i \yv_{i:\lambda},\;\text{where }\yv_{i:\lambda}=(\xv_{i:\lambda}-\bm{m})/\sigma$ \quad \hfill \textit{Selection/Recombination}
\State $\bm{m} \leftarrow \bm{m} + \sigma \yv_w$ \quad \hfill \textit{Update $\bm{m}$}
\State $\bm{p}_{\bm{C}} \leftarrow (1-c_{\bm{C}})\bm{p}_{\bm{C}} + \sqrt{c_{\bm{C}}(2-c_{\bm{C}})\mu_w}\yv_w$ \quad \hfill \textit{Cumulation of $\bm{C}$}
\State $\bm{p}_{\sigma} \leftarrow (1-c_{\sigma})\bm{p}_{\sigma} + \sqrt{c_{\sigma}(2-c_{\sigma})\mu_w} \bm{C}^{-1/2} \yv_w$ \quad \hfill \textit{Cumulation of $\sigma$}
\State $\bm{C} \leftarrow (1-c_1-c_{\mu}\sum w_j) \bm{C} + c_1 \bm{p}_{\bm{C}} \bm{p}_{\bm{C}}^\top + c_{\mu} \sum_{i=1}^{\mu} w_i \yv_{i:\lambda} \yv_{i:\lambda}^\top$ \quad \hfill \textit{Update $\bm{C}$}
...
\begin{framev}[fs=footnotesize]{CMA-ES: Wrap Up - Default Values}
...
\end{itemizeS}
with $\mu_w=\bigl(\frac{||\wv||_1}{||\wv||_2}\bigr)=\frac{(\sum_{i=1}^\mu |w_i|)^2}{\sum_{i=1}^\mu w_i^2} = \frac{1}{\sum_{i=1}^\mu w_i^2}$ and \textcolor{blue}{typical default parameter values}.
\spacer
\begin{itemizeS}
...
\begin{framev}{CMA-ES: Wrap Up}
\imageFixed{1.5cm}{1.5cm}[0.9]{figure_man/cmaes/cmaes_muds_\i.png}
\end{framev}
}
...
\comment[NOTE]{CK}{Sollte d sein (eine n-dimensionale Funktion hat n parameter/argumente und ist keine n-wertige Funktion)}
...
\comment[NOTE]{BB}{stopping criterion: besser sagen wie das gfeht}
\end{framei}


\begin{framei}[fs=footnotesize]{CMA-ES: Wrap Up - Benchmark EAs}
\item \textbf{Example:} Black-box optimization of 25 benchmark functions under thoroughly defined experimental and recording conditions for the 2005 IEEE Congress on Evolutionary Computation: Session on Real-Parameter Optimization. \furtherreading{SUGANTHANETAL2005CEC}\\
\spacer
17 papers were submitted, 11 were accepted, thereunder hybrid methods.\\
\textbf{Two of the Algorithms:}
\item L-CMA-ES: A CMA evolution strategy with small population size and small initial step-size to emphasize on local search characteristics. Independent restarts are conducted until the target function value is reached or the maximum number of function evaluations is exceeded. \furtherreading{AUGERHANSEN2005LOCALSEARCH}
\comment[NOTE]{BB}{diese varianten kennt auch keiner. es ist schon gut den echten benchmark zu nehmen, oder doch einen eigenen? und das eher kurz sagen was da rauskomt?}
\item G-CMA-ES: A CMA evolution strategy restarted with increasing population size (IPOP). Independent restarts are conducted with increasing population size until the target function value is reached or the maximum number of function evaluations is exceeded. With the initial small population size the algorithm converges fast, with the succeeding larger population sizes the global search performance is emphasized in subsequent restarts. \furtherreading{AUGERHANSEN2005IPOP}
\end{framei}


\begin{framev}[fs=footnotesize]{CMA-ES: Wrap Up - Benchmark EAs}
\imageC[0.9]{figure_man/cmaes/cmaes_benchmark.png}
\comment[NOTE]{BB}{blöd dass die algos alle unbekannt sind}
\begin{itemizeS}
