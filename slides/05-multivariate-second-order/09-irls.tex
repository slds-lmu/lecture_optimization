...
\item We now look at L2-regularized ERM problems with a \textbf{linear predictor} and a loss that is \textbf{separable over the observations}:
\begin{itemizeM}
...
\end{aligned}
$$
\item Here, $\tilde{\mathbf{f}}$ is called the \textbf{working response} (assuming $d_i > 0$), which will take the role of a pseudo-target; entry-wise
$$
...
\begin{framei}[fs=small]{Step 3: Newton step as weighted least squares}
\item This Newton iterate solves a \textbf{regularized weighted least squares} problem with working responses $\tilde{f}^{(i)}$ as targets and weights $d_i$:
$$
...
\item Since, in this specific case, neither $\mathbf{D}$ nor $\tilde{\mathbf{f}}$ depends on $\thetat$, the problem is solved in \textbf{one} Newton step (from any starting point)
\end{framei}
