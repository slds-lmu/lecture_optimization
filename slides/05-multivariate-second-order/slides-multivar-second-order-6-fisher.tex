...
$$\thetatn=\thetat+\E[-\nabla^{2}\logl(\thetat)]^{-1}\nabla\logl(\thetat)$$
\comment{CK}{expectation with respect to which random variable/distribtion?}
\end{framei}
...
\comment{CK}{Y does not appear in hessian anymore (randomness of y cond. on x)}
\vfill
...
\comment{TP}{this slide a_i and a used thats confusing}
\item $y|\xv$ belongs to an \textbf{exponential family} with density:
\comment{TP}{actually exponential dispersion family}
