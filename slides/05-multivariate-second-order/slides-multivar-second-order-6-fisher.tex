...
\comment[NOTE]{CK}{
expected gradient evaluated at true parameters is zero and hence squared expectation is variance of score
https://math.stackexchange.com/questions/3585130/why-is-the-fisher-information-matrix-both-an-expected-outer-product-and-a-hessia
...
$$\thetatn=\thetat+\E[-\nabla^{2}\logl(\thetat)]^{-1}\nabla\logl(\thetat)$$
\comment[NOTE]{CK}{expectation with respect to which random variable/distribtion?}
\end{framei}
...
\comment[NOTE]{CK}{Y does not appear in hessian anymore (randomness of y cond. on x)}
\vfill
...
\comment[NOTE]{TP}{this slide a_i and a used thats confusing}
\item $y|\xv$ belongs to an \textbf{exponential family} with density:
\comment[NOTE]{TP}{actually exponential dispersion family}
...
\comment[NOTE]{CK}{
mention glm and that many practical optimizers use IRLS for first iterations and switches over to newton-raphson
comment by tom wenseleers: https://stats.stackexchange.com/questions/205/why-do-we-make-a-big-fuss-about-using-fisher-scoring-when-we-fit-a-glm
}
