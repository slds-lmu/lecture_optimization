...
\item If $k$-th derivative of $f$ exists and is continuous on $\mathcal{S}$: $f$ is $
k$-times continuously differentiable; $f \in \mathcal{C}^k$ (class of $k$-times continuous differentiable functions)
...
\begin{itemize}
	\item Gradient is orthogonal to level curves / surfaces of a function
...
Let $f: \mathcal{S} \to \R^m$ be a vector-valued function with components $f_1, f_2, ..., f_m$. \textbf{Jacobian} matrix as generalization of gradient:  
