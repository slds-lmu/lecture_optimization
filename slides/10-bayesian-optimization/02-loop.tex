...
\begin{framei}{Optimization via Surrogate Modeling}
\item[] \textbf{Starting point:}
\item We do not know the objective function $f: \S \to \R$
\item But we can evaluate $f$ for a few different inputs $\xv \in \S$
...
\vfill
\imageC[0.5]{figure/loop_1.png}
\end{framei}


\begin{framei}{Initial Design}
...
\imageC[0.75]{figure/init_0.png}
\imageC[0.75]{figure/init_1.png}
...
\begin{framev}[align=top]{Surrogate Modeling}
Running example = minimize this \enquote{black-box}:
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_0.png}
\end{framev}
...
$\Rightarrow$ As we can eval $f$ without noise, we fit an interpolator
\end{enumerate}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_2.png}
\end{framev}
...
\begin{enumerate}
\setcounter{enumi}{1}
\item Instead of the expensive $f$, we optimize the cheap surrogate $\fh$ (blue) to \textbf{propose} a new point (red) for evaluation 
\end{enumerate}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_3.png}
\end{framev}
...
\end{enumerate}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_4.png}
\end{framev}
...
\end{itemize}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_5.png}
\end{framev}
...
\end{itemize}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_6.png}
\end{framev}
...
\end{itemize}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_7.png}
\end{framev}
...
\end{itemize}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_8.png}
\end{framev}
...
\end{itemize}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_9.png}
\end{framev}
...
$\Rightarrow$ We observe that the algorithm converged
\end{itemize}
\imageFixed{2.5cm}{3cm}[0.8]{figure/loop_10.png}
\end{framev}
...
\item Optimize the surrogate model $\fh$ to obtain a new point $\xvsi[t+1] \coloneqq \argmin_{\xv \in \S} \fh(\xv)$
