...
\imageC[1]{figure_man/noisy_2.png}
}
\vfill
$$
Y(\xv) ~|~ \xv, \Dt \sim \normal(\fh(\xv), \sh^2(\xv))
...
with
$$
\fh(\xv) = k(\xv)^T (\bm{K} + \sigma_{\eps}^2\mathbf{I}_t)^{-1} \bm{y}
$$
$$
\sh^2(\xv) = k(\xv, \xv) - k(\xv)^T (\bm{K} + \sigma_{\eps}^2\mathbf{I}_t)^{-1} k(\xv)
$$
\end{framev}


\begin{framev}{Noisy Acquisition Functions: AEI}
\textbf{Augmented Expected Improvement} (\sourceref{https://link.springer.com/article/10.1007/s10898-005-2454-3})
$$
a_{\text{AEI}}(\xv) = a_{\text{EI}_{f_{\min_*}}}(\xv) \Bigg(1- \frac{\sigma_{\eps}}{\sqrt{\sh^2(\xv)+ \sigma_{\eps}^2}}\Bigg).
$$
...
\spacer
$\sigma_{\eps}^2$ is the nugget-effect as estimated by the GP regression.
\end{framev}


\begin{framev}{Noisy Acquisition Functions: AEI}
\splitVTT[0.5]{
In addition, it takes into account the nugget-effect $\sigma_{\eps}^2$ by a penalty term:
$$
\Bigg(1- \frac{\sigma_\eps}{\sqrt{\sh^2(\xv)+ \sigma_\eps^2}}\Bigg)
$$
The penalty is justified to \enquote{account for the diminishing return of additional replicates as the predictions become more accurate} (\sourceref{https://link.springer.com/article/10.1007/s10898-005-2454-3})
}{
\imageC[1]{figure_man/noisy_3.png}
}
\vfill
\begin{itemize}
...
\imageC[1]{figure_man/noisy_4.png}
}
\end{framev}


\begin{framev}[fs=small]{Reinterpolation}
\begin{algorithm}[H]
...
\begin{framev}{Identification of final best point}
...
\end{itemize}
\vfill
\imageC[0.5]{figure_man/noisy_5.png}
\end{framev}
