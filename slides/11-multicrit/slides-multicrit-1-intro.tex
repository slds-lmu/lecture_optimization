...
\begin{framei}{Finding Pareto-optimal solutions VIII}
    ...
    \\[0.5cm]
    \textbf{Proof.} Suppose $\xv^*$ is not Pareto-optimal. Then there exists $\xv \in \mathcal{S}$, such that $\cost_i(\xv) \leq \cost_i(\xv^*)$ for all $i = 1, \dots, m$ and $\cost_j(\xv) < \cost_j(\xv^*)$ for at least one $j \in \{1, \dots, m\}$.
    Since $\xv^*$ is feasible for the $\epsilon$-constraint problem, it follows that $\cost_i(\xv) \leq \cost_i(\xv^*) \leq \epsilon_i$, so $\xv$ is also feasible.
    Moreover, $\cost_1(\xv) \leq \cost_1(\xv^*)$, and since $\xv^*$ is the unique minimizer of $\text{cost}_1$, we must have $\xv = \xv^*$, which is a contradiction.
\end{framei}
