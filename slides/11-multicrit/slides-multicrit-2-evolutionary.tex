...
\image[1]{figure/NSGA2_steps.png}
}
\comment[NOTE]{CK}{needs new axis labels f1/f2}
...
\begin{enumerate}
\item Initialize and evaluate population $\mathcal{P}_0 \subset \S$ with $|\mathcal{P}_0| = \mu$.
\item Set $t \gets 0$.
\item Repeat until the stop criterion is fulfilled:
\begin{enumerate}
\item Select parents from $\mathcal{P}_t$ and generate offspring $\mathcal{Q}_t$, with $|\mathcal{Q}_t| = \lambda$.
\item Select $\mu$ survivors to form $\mathcal{P}_{t+1}$.
\item Set $t \gets t + 1$.
\end{enumerate}
\end{enumerate}
\end{algorithm}
...
\end{itemize}
\imageC[0.45]{figure_man/NSGA2.png}
...
\vfill
\image[1]{figure/NSGA2_CS1.png}
\comment[NOTE]{CK}{need f1/f2 as axis labels}
...
\caption{SMS-EMOA}
\begin{enumerate}
\item Generate initial population $\mathcal{P}_0$ of size $\mu$.
\item Set $t \gets 0$.
\item Repeat until termination:
\begin{enumerate}
\item Create exactly one offspring $\mathbf{q}$ by recombination and mutation.
\item Perform non-dominated sorting on $(\mathcal{P}_t \cup \{\mathbf{q}\})$.
\item Let $\mathcal{F}_k$ be the last front.
\item Set $\tilde{\xv} \gets \argmin_{\xv \in \mathcal{F}_k} \Delta s\bigl(\xv,\mathcal{F}_k\bigr)$.
\item Set $\mathcal{P}_{t+1} \gets \bigl(\mathcal{P}_t \cup \{\mathbf{q}\}\bigr)\setminus\{\tilde{\xv}\}$.
\item Set $t \gets t + 1$.
\end{enumerate}
\end{enumerate}
\end{algorithm}
