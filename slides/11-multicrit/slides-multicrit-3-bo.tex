...
Initialize data $\D^{(0)}$ with initial observations\;\\
\For{$t=1$ \KwTo $T$}{
Sample $w \in W = \bigl\{(w_1,\dots,w_m) \,\mid\, \sum w_i=1,\, w_i=\tfrac{l}{s}, l \in \{0,\dots,s\}\bigr\}$\;\\
Compute scalarization:
$c^{(t)} = \max_{i}\bigl(w_i \fkx[i]\bigr) + \rho \sum_{i}w_i\fkx[i]$\\
Fit predictive model $\fh^{(t)}$ on $\D^{(t-1)}$\;\\
Select next query point: $\xv^{(t)} \in \argmax_{\xv \in \S} u(\xv; \D^{(t-1)}, \fh^{(t)})$\\
Query $f(\xv^{(t)})$\;\\
Update data: 
$$
\D^{(t)} \leftarrow \D^{(t-1)}
...
Initialize data $\D^{(0)}$ with initial observations\;\\
\For{$t=1$ \KwTo $T$}{
Fit predictive models $\fh_1^{(t)}, \dots, \fh_m^{(t)}$ on $\D^{(t-1)}$\;\\
Select next query point:\\
$\xv^{(t)} \in \argmax_{\xv \in \S} u\!\Bigl(\xv;\, \D^{(t-1)},\, \fh_1^{(t)},\dots,\fh_m^{(t)} \Bigr)$
Query $f(\xv^{(t)})$\;\\
Update data: 
$$
\D^{(t)} \leftarrow \D^{(t-1)}
