...
predictive model $\hat{f}$, maximal number of function evaluations $T$}
\Result{Best configuration $\hat{\xv}$ (based on $\D$ or $\hat{f}$)}

Initialize data $\D^{(0)}$ with initial observations\;
\For{$t \gets 1$ \KwTo $T$}{
Fit predictive model $\hat{f}^{(t)}$ on $\D^{(t-1)}$\;

Select next query point:
$\xv^{(t)} 
\;\in\; 
\argmax_{\xv \in \S}
u\!\Bigl(\xv;\,\D^{(t-1)},\;\hat{f}^{(t)}\Bigr)$
...
\min_{\xv \in \S} \bigl(f_1(\xv), f_2(\xv), \dots, f_m(\xv)\bigr).
\]
...
\[
f = \max_{i=1,\dots,m}\bigl(w_i f_i(\xv)\bigr) 
+ \rho \sum_{i=1}^m w_i f_i(\xv),
\]
...
$\quad f = \max_{i=1,\dots,m}\bigl(w_i f_i(\xv)\bigr) 
+ \rho \sum_{i=1}^m w_i f_i(\xv)$,
\begin{scriptsize}
\begin{itemize}
\item The norm consists of two components:
\begin{itemize}
\item $\max_{i}\bigl(w_i f_i(\xv)\bigr)$ 
considers only the maximum weighted cost.
\item $\sum_{i} w_if_i(\xv)$ is the weighted sum of all costs.
\end{itemize}
...
$c^{(t)} = \max_{i}\bigl(w_i f_i(\xv)\bigr) 
+ \rho \sum_{i}w_if_i(\xv)$

Fit predictive model $\hat{f}^{(t)}$ on 
$\D^{(t-1)}$\;
...
\begin{itemize}
\item Fit $m$ single-objective surrogate models $\hat{f}_1, \dots, \hat{f}_m$.
...
\[
u_{EI, \mathcal{H}}(\xv) 
= \int_{-\infty}^{\infty} p\bigl(f \mid \xv\bigr)
\times \bigl[\,
...
\item $p(f \mid \xv)$ is the joint predictive density of all surrogate models at $\xv$.
...
predictive model(s) $\hat{f}_1,\dots,\hat{f}_m$,
