\documentclass[11pt,compress,t,notes=noshow,xcolor=table]{beamer}

\usepackage{algorithm}

\input{../../style/preamble}
\input{../../latex-math/basic-math}
\input{../../latex-math/basic-ml}
\input{../../latex-math/ml-mbo}
\input{../../latex-math/optim-basics}

\title{Optimization in Machine Learning}

\begin{document}

\titlemeta{
Multi-criteria Optimization
}{
Bayesian Optimization
}{
figure_man/hv_contribution.pdf
}{
\item Single-criteria recap
\item Multi-criteria introduction
\item Scalarization vs. HV-based methods
}

\begin{framev}{Recap: Bayesian Optimization}
\begin{block}{Advantages of BO}
\begin{itemize}
\item Sample efficient
\item Can handle noise
\item Native incorporation of priors
\item Does not require gradients
\item Theoretical guarantees
\end{itemize}
\end{block}
We will now extend BO to multiple cost functions.
\end{framev}


\begin{framev}{Recap: Bayesian Optimization}
\begin{center}
\begin{algorithm}[H]
\caption{Bayesian optimization loop}
\begin{algorithmic}[1]
\Require Search space $\S$, cost function $f$, acquisition function $u$, predictive model $\fh$, maximal number of function evaluations $T$
\Ensure Best configuration $\hat{\xv}$ (based on $\D$ or $\fh$)
\State Initialize data $\D^{(0)}$ with initial observations
\For{$t \gets 1$ to $T$}
\State Fit predictive model $\fh^{(t)}$ on $\D^{(t-1)}$
\State Select next query point:
\State $\xv^{(t)} \in \argmax_{\xv \in \S} u\!\Bigl(\xv;\,\D^{(t-1)},\;\fh^{(t)}\Bigr)$
\State Query $f(\xv^{(t)})$
\State Update data:
\State $\D^{(t)} \leftarrow \D^{(t-1)} \cup \{\langle \xv^{(t)},\;f(\xv^{(t)}) \rangle \}$
\EndFor
\end{algorithmic}
\end{algorithm}
\end{center}
\end{framev}


\begin{framev}{Multi-Criteria Bayesian Optimization}
\textbf{Goal}: Extend Bayesian optimization to multiple cost functions
$$
\min_{\xv \in \S}  \fx
\quad \Leftrightarrow \quad
\min_{\xv \in \S} \bigl(\fkx[1], \fkx[2], \dots, \fkx[m]\bigr).
$$\\
\spacer
There are two basic approaches:
\begin{enumerate}
\item Simplify the problem by scalarizing the cost functions, or
\item define acquisition functions for multiple cost functions.
\end{enumerate}
\end{framev}


\begin{framev}{Scalarization}
\textbf{Idea:} Aggregate all cost functions
$$
\min_{\xv \in \S} \sum_{i = 1}^m w_i \fkx[i]
\quad\quad
\text{with} \quad w_i \ge 0
$$
\begin{itemize}
\item \textbf{Obvious problem:} How to choose $w_1, \dots, w_m$?
\begin{itemize}
\item Expert knowledge?
\item Systematic variation?
\item Random variation?
\end{itemize}
\item If expert knowledge is not available a priori, we need to ensure that 
different trade-offs between cost functions are explored.
\item Simplifies multi-criteria optimization problem to single-objective 
$\longrightarrow$ standard Bayesian optimization can be used without 
adapting the general algorithm.
\end{itemize}
\end{framev}


\begin{framev}{Scalarization: ParEGO\furtherreading{KNOWLES2006PAREGO}}
Scalarize the cost functions using the augmented Chebyshev norm / achievement function
$$
f = \max_{i=1,\dots,m}\bigl(w_i \fkx[i]\bigr) 
+ \rho \sum_{i=1}^m w_i \fkx[i],
$$
\begin{itemize}
\item The weights $w \in W$ are drawn from
$$
W = 
\bigl\{ w = (w_1, \dots, w_m) \,\big|\,
\sum_{i=1}^m w_i = 1,\, w_i = \tfrac{l}{s},\, l \in \{0,\dots,s\}
\bigr\},
$$
with $\lvert W \rvert = \binom{s+m-1}{m-1}$.
\item New weights are drawn in every BO iteration.
\item $\rho$ is a small parameter (e.g. 0.05).
\item $s$ controls how many distinct weight vectors exist.
\end{itemize}
\end{framev}


\begin{framev}{Why the Chebyshev norm?}
\begin{center}
\imageC[0.85]{figure_man/parego_viz.png}
\end{center}
$\quad f = \max_{i=1,\dots,m}\bigl(w_i \fkx[i]\bigr) 
+ \rho \sum_{i=1}^m w_i \fkx[i]$,
\begin{scriptsize}
\begin{itemize}
\item The norm consists of two components:
\begin{itemize}
\item $\max_{i}\bigl(w_i \fkx[i]\bigr)$ 
considers only the maximum weighted cost.
\item $\sum_{i} w_i\fkx[i]$ is the weighted sum of all costs.
\end{itemize}
\item $\rho$ controls the trade-off between these parts.
\item By randomizing the weights $w$ in each iteration (and keeping $\rho$ small), 
extreme points in single cost functions can be explored.
\item One can prove that every solution of this scalarized problem is 
Pareto-optimal.
\end{itemize}
\end{scriptsize}
\end{framev}


\begin{framev}[fs=small]{ParEGO Algorithm}
\begin{center}
\begin{algorithm}[H]
\caption{ParEGO loop}
\begin{algorithmic}[1]
\Require Search space $\S$, cost function $f$, acquisition function $u$, predictive model $\fh$, maximal number of function evaluations $T$, $\rho$, $l$, $s$
\Ensure Best configuration $\hat{\xv}$ (according to $\D$ or $\fh$)
\State Initialize data $\D^{(0)}$ with initial observations
\For{$t = 1$ to $T$}
\State Sample $w \in W = \bigl\{(w_1,\dots,w_m) \,\mid\, \sum w_i = 1,\, w_i = \tfrac{l}{s}, l \in \{0,\dots,s\}\bigr\}$
\State Compute scalarization: $c^{(t)} = \max_{i}\bigl(w_i \fkx[i]\bigr) + \rho \sum_{i}w_i\fkx[i]$
\State Fit predictive model $\fh^{(t)}$ on $\D^{(t-1)}$
\State Select next query point: $\xv^{(t)} \in \argmax_{\xv \in \S} u(\xv; \D^{(t-1)}, \fh^{(t)})$
\State Query $f(\xv^{(t)})$
\State Update data: $\D^{(t)} \leftarrow \D^{(t-1)} \cup \{\langle \xv^{(t)}, f(\xv^{(t)}) \rangle\}$
\EndFor
\end{algorithmic}
\end{algorithm}
\end{center}
\end{framev}


\begin{framev}{HV-based Acquisition Functions}
\textbf{Idea:} Define acquisition function that directly models the contribution to the dominated hypervolume (HV):
$\max(0, \; S(\mathcal{P} \cup \xv, R) \;-\; S(\mathcal{P}, R))$
\begin{center}
\imageC[0.8]{figure_man/hv_contribution.pdf}
\end{center}
\begin{itemize}
\item Fit $m$ single-objective surrogate models $\fh_1, \dots, \fh_m$.
\item Acquisition function uses all surrogate models simultaneously.
\item Still a single-objective optimization problem (of acquisition itself).
\end{itemize}
\end{framev}


\begin{framev}{S-Metric Selection-based EGO}
Using the Lower Confidence Bound $u_{\text{LCB},1}(\xv),\dots,u_{\text{LCB},m}(\xv)$,
one can form an \emph{optimistic} estimate of the hypervolume contribution:
\begin{center}
\imageC[0.9]{figure_man/hv_contribution_2.pdf}
\end{center}
\textbf{Problem:} HV-contribution can be zero across large regions where the lower confidence bound is already dominated.
\begin{itemize}
\item Large zero-plateaus make optimization of the acquisition harder.
\item SMS-EGO adds an adaptive penalty in such dominated regions.
\end{itemize}
\spacer
This method is referred to as
\furtherreading{PONWEISER2008MULTI}.
\end{framev}


\begin{framev}{Further HV Acquisition Functions}
\textbf{Expected Hypervolume Improvement (EHI)} 
\furtherreading{YANG2019MULTI}:
$$
u_{EI, \mathcal{H}}(\xv) 
= \int_{-\infty}^{\infty} \pdf\bigl(f \mid \xv\bigr)
\times \bigl[\,
S(\mathcal{P} \cup \xv, R) \;-\; S(\mathcal{P}, R)
\bigr]\; df
$$
\begin{itemize}
\item Direct extension of $u_{EI}$ to the hypervolume criterion.
\item $\pdf(f \mid \xv)$ is the joint predictive density of all surrogate models at $\xv$.
\item With independent GPs per objective, this factorizes over $m$ univariate normals.
\item Exact integral feasible for $m \le 3$, else simulation-based.
\end{itemize}
\spacer
Further HV-based acquisitions:
\begin{itemize}
\item \textbf{Stepwise Uncertainty Reduction (SUR)} w.r.t.\ the probability of improvement.
\item \textbf{Expected Maximin Improvement (EMI)} w.r.t.\ the $\epsilon$-indicator.
\end{itemize}
\end{framev}


\begin{framev}{Hypervolume-based BO Algorithm}
\begin{center}
\begin{algorithm}[H]
\caption{Hypervolume-based Bayesian optimization loop}
\begin{algorithmic}[1]
\Require Search space $\S$, cost function $f$, acquisition function $u$, predictive model(s) $\fh_1,\dots,\fh_m$, maximal number of function evaluations $T$
\Ensure Best configuration $\hat{\xv}$
\State Initialize data $\D^{(0)}$ with initial observations
\For{$t = 1$ to $T$}
\State Fit predictive models $\fh_1^{(t)}, \dots, \fh_m^{(t)}$ on $\D^{(t-1)}$
\State Select next query point:
\State $\xv^{(t)} \in \argmax_{\xv \in \S} u\!\Bigl(\xv;\, \D^{(t-1)},\, \fh_1^{(t)},\dots,\fh_m^{(t)} \Bigr)$
\State Query $f(\xv^{(t)})$
\State Update data: $\D^{(t)} \leftarrow \D^{(t-1)} \cup \{\langle \xv^{(t)},f(\xv^{(t)})\rangle\}$
\EndFor
\end{algorithmic}
\end{algorithm}
\end{center}
\end{framev}

\endlecture
\end{document}
