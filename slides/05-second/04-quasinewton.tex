...
\comment[NOTE]{BB}{/https://www.stat.cmu.edu/~ryantibs/convexopt-F13/lectures/11-QuasiNewton.pdf}
...
\item Start point of \textbf{Quasi-Newton (QN) method} is (as with Newton-Raphson (NR)) a Taylor approximation of the gradient, except that the Hessian is replaced by a \textbf{positive definite (pd)} matrix $\A^{[t]}$:
\begin{alignat*}{4}
...
\item Select a starting point $\xv^{[0]}$ and initialize pd matrix $\A^{[0]}$ (can also be a diagonal matrix, a very rough approximation of the Hessian)
\item Calculate update direction by solving
$$
\A^{[t]} \bm{d}^{[t]} = - \nabla f(\xv^{[t]})
$$
and set $\xv^{[t+1]} = \xv^{[t]} + \alpha^{[t]} \bm{d}^{[t]}$ (Step size through backtracking)
\comment[NOTE]{BB}{wie genau? wir brauchen echt mehr zu step size control}
...
\item Low computational effort when solving the linear system
$$
\A^{[t]} \bm{d}^{[t]} = - \nabla f(\xv^{[t]})
$$
...
$$
\A^{[t+1]} = \A^{[t]} + \bm{B}^{[t]} = \A^{[t]} + \beta \bm{u}^{[t]}(\bm{u}^{[t]})^T
$$
...
\comment[NOTE]{BB}{müssen wir hier nicht mit den indizes etwas aufpassen? also dass x = x^t ist}
$$
\begin{aligned}
\nabla f(\xv) &\overset{!}{=} \nabla f(\xv^{[t+1]}) + \A^{[t+1]}(\xv - \xv^{[t+1]}) \\
\nabla f(\xv) &= \nabla f(\xv^{[t+1]}) +  (\A^{[t]} + \beta \bm{u}^{[t]}(\bm{u}^{[t]})^T)\underbrace{(\xv - \xv^{[t+1]})}_{:= \bm{s}^{[t+1]}} \\
\underbrace{\nabla f(\xv) - \nabla f(\xv^{[t+1]})}_{\bm{y}^{[t+1]}} &= (\A^{[t]} + \beta \bm{u}^{[t]} (\bm{u}^{[t]})^T) \bm{s}^{[t+1]} \\
...
$$
\comment[NOTE]{BB}{formel siehtr falsch aus? A muss inv werden, rest auch komisch?}
\end{itemizeS}
\vfill
...
\comment[NOTE]{BB}{zu pd: vorsicht behaupten wir das vorher?}
\end{itemizeS}
\end{framei}
