# Used in: slides/05-second/03-nrlimit.tex
#
# Saddle-point problem of Newton-Raphson (Aggarwal Ch. 5.6.2).
#
# NR_saddle_2d.png: the 2d saddle f(x1, x2) = x1^2 - x2^2 with H = diag(2, -2).
#   Newton-Raphson solves grad f = 0 and therefore jumps onto the saddle at the
#   origin in a single step from any starting point (f is quadratic, so the
#   quadratic model is exact). Gradient descent is repelled along the negative
#   curvature direction x2 and escapes.
# NR_saddle_1d.png: the 1d inflection point of the cubic f(x) = x^3. The second-order
...
f_saddle = function(x1, x2) x1^2 - x2^2

grid = expand.grid(
  x1 = seq(-2, 2, length.out = 300),
  x2 = seq(-2, 2, length.out = 300)
)
grid$z = f_saddle(grid$x1, grid$x2)
...
# Newton-Raphson: d = -H^{-1} grad f = -(x1, x2) -> lands on the saddle
nr_path = data.frame(
  x1 = c(x_start[1L], 0),
  x2 = c(x_start[2L], 0),
...
saddle_plot = ggplot(grid, aes(x = x1, y = x2)) +
  ...
  labs(
    subtitle = expression(f(x[1], x[2]) == x[1]^2 - x[2]^2),
    x = expression(x[1]),
    y = expression(x[2])
  ) +
...
curve_levels = c("f", "model_pos", "model_neg")
# plotmath labels so the legend shows x^3 and x = -1 rather than literal "x^3"
curve_labels = expression(
  f(x) == x^3,
  paste("model at ", x == 1, ": convex, step to min"),
  paste("model at ", x == -1, ": concave, step to max")
)

curves = rbind(
  data.frame(x = xs, y = f_cubic(xs), which = "f"),
  data.frame(x = xs, y = model_at_pos(xs), which = "model_pos"),
  data.frame(x = xs, y = model_at_neg(xs), which = "model_neg")
)
curves$which = factor(curves$which, levels = curve_levels)

expansion_points = data.frame(x = c(1, -1), y = f_cubic(c(1, -1)))
...
taylor_plot = ggplot(curves, aes(x = x, y = y, colour = which, linetype = which)) +
  ...
  scale_colour_manual(
    values = c(f = "black", model_pos = "firebrick", model_neg = "darkorange"),
    labels = curve_labels,
    name = NULL
  ) +
  scale_linetype_manual(
    values = c(f = "solid", model_pos = "dashed", model_neg = "dashed"),
    labels = curve_labels,
    name = NULL
  ) +
